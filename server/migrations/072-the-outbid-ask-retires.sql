-- 072 · THE OUTBID ASK RETIRES
--
-- "These outbid notifications are not going away," said the manager, and he
-- was right: being outbid was BOTH a wire line (timestamped, cleared by
-- reading) and a standing ask (pinned, undismissable while the listing
-- stayed open). The same word twice, and one of them could not be put down.
-- The wire keeps the story - who went above you, and to what figure - and
-- the ask goes the way of the boys-leaving nag (070).
--
-- 070's body, with ask 4 removed. Everything else is unchanged.
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(row ORDER BY ord, ord2), '[]'::jsonb) INTO asks FROM (
    ...
    -- (4, the outbid ask, was retired by 072: the wire already carries the
    --  same news with the rival's name and the standing figure, timestamped
    --  and clearable, and a standing ask that cannot be dismissed sat pinned
    --  in the bell for as long as the listing stayed open - a nag, not news.)
