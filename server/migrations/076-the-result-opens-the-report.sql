-- 076 · THE RESULT OPENS THE REPORT
--
-- "Clicking on a match-over notification takes me to the set-lineup page
-- instead of the post-match page," said the manager, and the wire had been
-- pointing at the wrong room since the bell was built. A played match went
-- out as '#/match?id=<uuid>' - but #/match is the LIVE matchday centre, and
-- nothing in the client reads an ?id= off it (only #/friendly?id= is parsed).
-- So the address decayed to a bare #/match and the reader landed on whatever
-- that tab had pending: his teamsheet, for a match already in the books.
--
-- The room that holds a finished league match is the report, and it names
-- itself by nation and the World Service's match id - '#/report?n=eng&w=<id>'
-- - which is exactly what the fixtures list has always linked to. The wire
-- says the same thing now.
--
-- 075's body, with one destination corrected. Everything else is unchanged.
