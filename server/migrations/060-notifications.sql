-- 060-notifications.sql — WHAT HAPPENED, AND WHAT IS WAITING FOR YOU.
--
-- The world had a bell already. It was gated on SYNC.started && LG - the
-- retired private-league mode - so in the served world it never appeared; it
-- knew about exactly one thing, friendly challenges; and it remembered what
...
-- game would not have said a word.
--
-- HOW FROM THE PAVILION DOES IT, and what this copies. FTP's News is an inbox
-- of dated, categorised messages written at the update: your match, your
-- training, your transfers, your youth, your finances, your national call-up.
-- The categories are not decoration - they are how a manager decides what to
-- read when he has thirty seconds. Hitwicket adds the other half: a bell that
-- pins the things that need a decision NOW, because a management game whose
-- deadlines pass quietly is a management game you stop playing.
--
-- So there are two kinds here and the difference is the whole design:
--
--   NEWS is what happened. You read it at leisure and it goes stale.
--   ASKS are what is waiting on you, and they do not go away by being read -
--   only by being done. An unanswered scout is still unanswered tomorrow.
--
-- NOTHING IS STORED THAT THE RECORD ALREADY KNOWS. 037 established the shape:
-- a club's feed is a UNION over the record, computed live, and "it can never
-- drift from what actually happened, because it IS what happened". Every line
-- below obeys that. A match notification is the match row; a step-up is the
-- book of the nets; an unanswered scout is the scout row with a null decision.
-- Re-run the world and the notifications come back identical.
--
-- THE ONE EXCEPTION IS DELIBERATE. Whether you have READ something is not a
-- fact about the world, it is a decision you made - like an order, a training
-- plan or a claim - and no derivation can recover it. So exactly one thing is
-- written down: a watermark per manager. Everything else is arithmetic.
...
-- MONEY IS WRITTEN THE WAY MONEY IS READ. "The hammer fell at 480000" is a
-- number a manager has to count the digits of; "$480,000" is a price. One
-- helper so every line in the feed says it the same way.
CREATE OR REPLACE FUNCTION notif_money(p bigint)
RETURNS text LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE
    WHEN coalesce(p, 0) >= 1000000 THEN '$' || trim(to_char(p / 1000000.0, 'FM999990.0')) || 'm'
    WHEN coalesce(p, 0) >= 1000 THEN '$' || trim(to_char(round(p / 1000.0), 'FM999990')) || 'k'
    ELSE '$' || coalesce(p, 0)::text END
$$;
    ...
    SELECT * FROM (
      ...
      SELECT (extract(epoch FROM m.played_at) * 1000)::bigint AS at,
             jsonb_build_object('at', (extract(epoch FROM m.played_at) * 1000)::bigint,
               'kind', 'match', 'fresh', m.played_at > seen,
...
-- ---------------------------------------------------------------------------
-- READ IT. The watermark moves to now; asks are untouched, because an ask is
-- not answered by being looked at.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  INSERT INTO notif_seen(user_id, seen_at) VALUES (u, now())
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'authenticated') THEN
    GRANT EXECUTE ON FUNCTION public.world_my_notifications(int) TO authenticated;
    GRANT EXECUTE ON FUNCTION public.world_notifications_seen() TO authenticated;
  END IF;
END $$;
