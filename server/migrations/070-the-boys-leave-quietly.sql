-- 070 · THE BOYS LEAVE QUIETLY
--
-- The manager asked for the "boys leave at the turning of the year" ask to
-- go. It was a standing item from the day the feed was built (060): any colt
-- of twenty lit the bell until the season turned or a shirt was signed, and
-- because an ask survives being read by design, it sat in the popover for
-- weeks as a thing that could not be dismissed. The squad room already
-- prints every boy's age, and the promote button sits beside it; the bell
-- does not need to say it every day as well.
--
-- 069's body, with ask 7 removed. Everything else is unchanged.
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(row ORDER BY ord, ord2), '[]'::jsonb) INTO asks FROM (
    ...
    -- (7, the boys-leaving warning, was retired by 070: the squad room
    --  prints every boy's age beside his promote button, and a standing ask
    --  that cannot be dismissed is a nag, not news.)
  ) a;
...
-- ---------------------------------------------------------------------------
-- THE WATERMARK KEEPS THE SAME CLOCK AS THE NEWS. seen_at was stamped with
-- now() - the wall clock - while every news item above is stamped off
-- now_ms(), the world's own clock. In production the two agree to the
-- millisecond; anywhere the world clock is steered (every test harness) they
-- do not, and news stamped ahead of the wall clock could never be marked
-- read. One clock, everywhere.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  INSERT INTO notif_seen(user_id, seen_at) VALUES (u, to_timestamp(now_ms() / 1000.0))
    ON CONFLICT (user_id) DO UPDATE SET seen_at = to_timestamp(now_ms() / 1000.0);
