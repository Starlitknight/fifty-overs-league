...
test('the world shape comes from the shipped build and the clocks agree', () => {
  ...
  assert.equal(cfgs.length, 16, '16 nations - the Full Members and the strongest Associates');
  assert.ok(cfgs.some(c => c.id === 'eng'), 'England present');
  for (const c of cfgs) {
    assert.equal(c.clubs.length, 16, c.id + ' seats sixteen clubs in two divisions');
...
test('initWorld founds all 19 leagues', async () => {
  ...
  assert.equal(r.countries.length, 16);
  ...
  assert.equal(nC.rows[0].n, 16);
  assert.equal(nK.rows[0].n, 256);
  assert.equal(nS.rows[0].n, 16);
});

test('expandWorld re-founds a missing country with byte-identical squads', async () => {
  const beforeSquads = (await pool.query("SELECT slot, name, squad FROM clubs WHERE country_id='nep' ORDER BY slot")).rows;
  assert.equal(beforeSquads.length, 16);
  await pool.query("DELETE FROM seasons WHERE country_id='nep'");
  await pool.query("DELETE FROM clubs WHERE country_id='nep'");
  await pool.query("DELETE FROM countries WHERE id='nep'");
  const x = await expandWorld(pool, { now: T0, host });
  assert.deepEqual(x.added, ['nep'], 'only the missing country is founded');
  const afterSquads = (await pool.query("SELECT slot, name, squad FROM clubs WHERE country_id='nep' ORDER BY slot")).rows;
  assert.equal(afterSquads.length, 16);
  for (let i = 0; i < 16; i++) {
...
test('one runAllDue settles round 1 across the whole planet', async () => {
  ...
  assert.equal(leagues.rows[0].n, 16, 'one league snapshot per nation');
});
