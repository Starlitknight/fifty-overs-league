  ...
  for (const ch of due) {
    try {
      ...
      const result = await page.evaluate(({ snap, ch }) => {
        ...
        return { result_text: r.result_text, winner: r.winner_team, mom: r.mom, scorecard: r.scorecard, worm: r.worm, log: r.log, ratings_html: r.ratings_html, fantasy: r.fantasy, pitch: r.pitch, meta: r.meta };
      }, { snap: st.snapshot, ch });
