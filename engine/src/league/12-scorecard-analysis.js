    ...
    try {
      ...
      var openCard = function (ci) {
        ...
        cardEl.innerHTML =
          ...
          "<button type='button' class='tc-enter' data-tc-enter>Visit the ground &#9654;</button>" +
  ...
  // ==========================================================================
  var FO_WORLD_PINS = {
    ire: [42.4, 22.4], eng: [46.3, 24.6], ned: [48.9, 27.2], gt: [43.9, 31.4],
    win: [26.5, 42], rsa: [53.5, 71.5], zim: [54.8, 63.5],
    aus: [86.5, 71], nzl: [92.4, 79.8], slk: [68.8, 55.5], sub: [66.4, 45.5],
    pak: [63.2, 39.5], afg: [60.2, 35.6]
  };
  function foRenderWorld() {
    try {
      ...
      var pins = FO_CX_REGIONS.map(function (r, i) {
        ...
        var cls = "fo-wp" + (r.final ? " crown" : "") + (done ? " done" : live ? " live" : "") + (lockF ? " lockf" : "");
        return "<button type='button' class='" + cls + "' data-wr='" + i + "' style='left:" + p[0] + "%;top:" + p[1] + "%;--wac:" + r.ac + "'>" +
          "<span class='dot'>" + (done ? "&#10003;" : r.final ? "&#9818;" : "&#9873;") + "</span>" +
      ...
      page.innerHTML =
        ...
        "<div class='fo-world-hint' id='fo-world-hint'>Drag to roam &middot; tap a nation to tour it</div>" +
      ...
      // centre the painting; drag to roam with true 16:9 clamps
      ...
      var panW = function (dx, dy) {
        ...
        dx = Math.max(-bx, Math.min(bx, dx)); dy = Math.max(-by, Math.min(by, dy));
        mapEl.style.transform = "translate(calc(-50% + " + dx + "px),calc(-50% + " + dy + "px))";
  ...
  try {
    ...
    woCss.textContent =
      ...
      // nation medallions
      "html body #page .fo-wp,html body.ftpskin #page .fo-wp{position:absolute;transform:translate(-50%,-50%);display:flex;flex-direction:column;align-items:center;gap:5px;background:none !important;border:none !important;box-shadow:none !important;padding:0;cursor:pointer;z-index:3}" +
      ".fo-wp .dot{width:34px;height:34px;display:flex;align-items:center;justify-content:center;border-radius:50%;background:rgba(6,16,32,.82);border:2.5px solid var(--wac,#F0B94E);color:#fff;font-size:15px;box-shadow:0 4px 14px rgba(0,0,0,.55),0 0 0 3px rgba(6,16,32,.35);transition:transform .15s ease,box-shadow .15s ease}" +
      ".fo-wp:hover .dot{transform:scale(1.16)}" +
      ".fo-wp.sel .dot{box-shadow:0 0 0 4px rgba(240,185,78,.5),0 0 22px rgba(240,185,78,.8),0 4px 14px rgba(0,0,0,.55)}" +
      ".fo-wp.done .dot{background:#3E9455;border-color:#8fe3a4}" +
      ".fo-wp.live .dot{animation:foWpPulse 2s ease-out infinite}" +
      "@keyframes foWpPulse{0%{box-shadow:0 0 0 0 rgba(240,185,78,.55),0 4px 14px rgba(0,0,0,.55)}70%{box-shadow:0 0 0 13px rgba(240,185,78,0),0 4px 14px rgba(0,0,0,.55)}100%{box-shadow:0 0 0 0 rgba(240,185,78,0),0 4px 14px rgba(0,0,0,.55)}}" +
      ".fo-wp.crown .dot{width:42px;height:42px;font-size:20px;border-color:#F3D37A;color:#F3D37A}" +
      ".fo-wp.lockf .dot{opacity:.75;filter:saturate(.5)}" +
      ".fo-wp .nm{font-family:Oswald,sans-serif;font-size:10.5px;font-weight:600;letter-spacing:2px;text-transform:uppercase;color:#F3EEDF;white-space:nowrap;text-shadow:0 1px 4px rgba(0,0,0,.95),0 0 8px rgba(0,0,0,.7)}" +
      ...
      // the nation card
      ".fo-world-card{position:absolute;z-index:6;left:26px;bottom:26px;width:min(470px,calc(100vw - 52px));display:none;background:rgba(7,18,36,.88);border:1px solid rgba(255,255,255,.2);border-radius:18px;overflow:hidden;backdrop-filter:blur(14px);box-shadow:0 26px 70px rgba(0,0,0,.6)}" +
      ...
      ".fo-world-card .wc-art{flex:0 0 158px;position:relative}" +
      ...
      ".fo-world-card .wc-bd{flex:1;min-width:0;padding:15px 16px 14px;color:#fff}" +
      ".fo-world-card .wc-eb{font-family:Oswald,sans-serif;font-size:9.5px;font-weight:600;letter-spacing:2.4px;text-transform:uppercase;color:var(--wac,#F3D37A)}" +
      ".fo-world-card .wc-h2{font-family:Oswald,sans-serif;font-weight:600;font-size:25px;letter-spacing:1.2px;text-transform:uppercase;margin:2px 0 4px}" +
      ".fo-world-card .wc-boss{font-size:12.5px;color:rgba(255,255,255,.92)}" +
      ".fo-world-card .wc-boss b{color:#F3D37A}" +
      ".fo-world-card .wc-line{font-family:Georgia,serif;font-size:12.5px;line-height:1.45;color:rgba(255,255,255,.85);margin-top:5px}" +
      ".fo-world-card .wc-prog{display:flex;align-items:center;gap:9px;margin-top:9px;flex-wrap:wrap}" +
      ".fo-world-card .wc-chip{font-family:Oswald,sans-serif;font-size:9.5px;font-weight:600;letter-spacing:1.8px;text-transform:uppercase;padding:5px 10px;border-radius:999px;border:1px solid rgba(255,255,255,.25);color:#fff}" +
      ...
      ".fo-world-card .wcd{display:inline-flex;gap:4px}" +
      ".fo-world-card .wcd i{width:8px;height:8px;border-radius:50%;background:rgba(255,255,255,.22)}" +
      ".fo-world-card .wcd i.f{background:#F0B94E}" +
      ".fo-world-card .wc-row{margin-top:11px}" +
      "html body .fo-world-card .wc-tour{font-family:Oswald,sans-serif !important;font-weight:600 !important;font-size:12px;letter-spacing:2.2px;text-transform:uppercase;background:linear-gradient(180deg,#F0B94E,#C9A24B) !important;color:#101B2D !important;border:none !important;border-radius:999px;padding:11px 22px;cursor:pointer;box-shadow:0 4px 0 rgba(16,27,45,.3),0 8px 20px rgba(201,162,75,.3) !important}" +
      "html body .fo-world-card .wc-tour:hover{filter:brightness(1.06)}" +
      "@media(max-width:700px){.fo-world-card{left:10px;right:10px;bottom:12px;width:auto}.fo-world-card .wc-art{flex-basis:112px}.fo-world-card .wc-h2{font-size:20px}}";
