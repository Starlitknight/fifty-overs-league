  ...
  function foChalAttach(ch, done) {
    rpc("challenge_set_orders", { p_id: ch.id, p_club: userTeam().name, p_orders: App.orders })
      .then(function () {
        ...
        var s = ((e && e.message) || e) + "";
        if (/lock/i.test(s) || /already played/i.test(s)) say("Lineups locked an hour before kickoff. Kickoff: " + foChalWhen(ch) + ".");
        else say(e);
        if (done) done(false);
      });
  }
  ...
  function foFrLiveScore(r, tk, upto) {
    try {
      ...
      var bat = orderBat.map(function (n) {
        var x = seenBat[n];
        var st2 = atCrease[n] ? "<span style='color:#177A57;font-weight:700'>batting</span>" : E(outTxt[n] && outTxt[n] !== "not out" ? outTxt[n] : "");
        ...
        window.__foFrRows.forEach(function (c) {
          ...
          if (c.status === "pending" && !mineSent) act = "<button class='mini fo-ch-acc' data-id='" + c.id + "'>Accept</button> <button class='mini fo-ch-dec' data-id='" + c.id + "'>Decline</button>";
            ...
            act = "<span class='fo-frs-on'>ON</span> " + (lkd
              ? "<span class='small'>&#128274; lineups locked" + (hasOrd ? " \u00b7 <span style='color:#177A57'>&#10003; yours is in</span>" : " \u00b7 auto XI plays") + "</span> <a href='#/friendly?id=" + c.id + "'>Preview &rsaquo;</a>"
              : "<button class='mini fo-ch-ord' data-id='" + c.id + "'>" + (hasOrd ? "Edit lineup" : "Prepare lineup") + "</button>" + (hasOrd ? " <span class='small' style='color:#177A57'>&#10003; lineup attached</span>" : ""));
          }
  ...
  function foTrainingPage() {
    ...
    if (howB) howB.addEventListener("click", function () {
      ...
      m.innerHTML = "<div class='fo-modal-card' style='max-width:720px;max-height:84vh;overflow:auto'><div class='fo-modal-eyebrow'>Development centre</div><h3>How training works</h3>" +
        ...
        "<div class='small' style='margin:2px 0 8px;color:#6A6354'>Every session splits its progress across these skills, in these proportions.</div>" +
        foProgExplainHTML() +
  ...
  function foThSetSpeed(m) {
    try {
      window.__foThMult = m;
      try {
        var sc = document.getElementById("fo-mob-spdchip");
        if (sc) sc.textContent = m + "\u00d7";
        document.body.classList.remove("fo-spd-open");
        clearTimeout(window.__foSpdT);
      } catch (eSc) {}
  ...
  // Pure presentation - it only chooses what is visible and which engine
  // tab renders; the broadcast itself never pauses for it.
  function foMobIsOn() {
        ...
        if (st === "score") {
          try {
            if (typeof UI !== "undefined") {
              if (["Scorecard", "Commentary", "Charts"].indexOf(UI.matchTab) < 0) UI.matchTab = "Scorecard";
  ...
  // the field-talk row lives in the oval pane, on every screen size; if a
  // re-render orphaned it, send it home under the svg
  function foMobTalkHome() {
    try {
      var talk = document.getElementById("fo-mst-talk"), ov = document.getElementById("fo-oval");
      if (talk && ov && talk.parentNode !== ov) {
  ...
  function foThChrome(on) {
    ...
    if (!document.getElementById("fo-mob-nav")) {
      ...
      sh9.innerHTML = "<div class='sh-grip'></div>";
      ...
      sc9.addEventListener("click", function () {
        document.body.classList.toggle("fo-spd-open");
        clearTimeout(window.__foSpdT);
        if (document.body.classList.contains("fo-spd-open"))
          window.__foSpdT = setTimeout(function () { try { document.body.classList.remove("fo-spd-open"); } catch (e6) {} }, 5000);
      });
      ...
      var drMove9 = function (e8) {
        ...
        if (dy9 > 44) { drY9 = null; foMobSheet("live"); }
      };
  ...
  function foDashUI(o) {
    ...
    for (var i = 0; i < M.log.length; i++) {
      ...
      if (nRows < 11) {
        var wkF = (typeof isWkt === "function" && isWkt(Lg.out));
        var txF = String(Lg.txt || ""), whoF = "", mF = /^(.{2,40}? to .{2,30}?)\s*:\s*(.*)$/.exec(txF);
        if (mF) { whoF = mF[1]; txF = mF[2]; }
        rows += "<div class='db-cr" + (first ? " latest" : "") + (wkF ? " wk" : Lg.out === "4" ? " r4" : Lg.out === "6" ? " r6" : "") + "'>" +
          "<b>" + E(Lg.no || "") + "</b>" + foDashBall(Lg.out) +
          "<p>" + (whoF ? "<b class='who'>" + E(whoF) + "</b> " : "") + E(txF) +
          (first && o.kphTx ? " <span class='kph'>" + o.kphTx + "</span>" : "") + "</p></div>";
        nRows++; first = false;
      }
    }
    ...
    // runs per over from the worm the umpire banks every ball; the over a
    // wicket fell in is dotted
    ...
    var barLeft = M.done
      ...
      : M.target
        ? "<span class='bt'>" + E(String(batTeam).toUpperCase()) + " NEED <i>" + need + "</i> FROM " + Math.floor(ballsLeft / 6) + "." + (ballsLeft % 6) + " OVERS</span>"
    ...
    return "<div class='fo-db-wrap'>" +
      ...
      (M.target
        ? "<div class='st tst'><span class='k'>Target</span><b>" + M.target + "</b></div>" +
          "<div class='st'><span class='k'>Remaining</span><b class='or'>" + need + "</b><span class='k'>off " + Math.floor(ballsLeft / 6) + "." + (ballsLeft % 6) + " ov</span></div>" +
          "<div class='st'><span class='k'>Req. rate</span><b>" + (req || "&ndash;") + "</b></div>"
      ...
      "<button type='button' id='fo-db-act'>Scorecard</button></div>";
  }
  // THE FULL COMMENTARY, as its own page: every ball the umpire wrote, both
  // innings, end-of-over summaries between them, and filters to read it the
  // way a scorer would - everything straight from M.log and the worm.
  var FO_DASH_CFILT = [["all", "All"], ["runs", "Runs"], ["bound", "4s & 6s"], ["wkt", "Wickets"], ["mile", "Milestones"]];
  function foDashCommFull(refresh) {
    try {
      var old = document.getElementById("fo-commpage");
      if (refresh && !old) return;
      var filt = (old && old.getAttribute("data-filt")) || "all";
      var keepScroll = refresh && old ? old.querySelector(".cp-list").scrollTop : 0;
      var isW = function (oo) { return typeof isWkt === "function" && isWkt(oo); };
      var pass = function (Lg) {
        if (Lg.mile) return filt === "all" || filt === "mile";
        if (filt === "mile") return false;
        if (filt === "all") return true;
        if (filt === "wkt") return isW(Lg.out);
        if (filt === "bound") return Lg.out === "4" || Lg.out === "6";
        if (filt === "runs") return ["1", "2", "3", "4", "6"].indexOf(Lg.out) >= 0;
        return true;
      };
      // the umpire's own milestone rows already tell the end of each over,
      // the fifties and the falls - the page only adds the innings breaks
      var rows = "", prevInn = null, shown = 0;
      for (var i = 0; i < M.log.length; i++) {
        var Lg = M.log[i];
        // the log reads newest-first: a band announces the section BELOW it
        if (prevInn !== null && Lg.inn !== prevInn) {
          rows += "<div class='cp-band inn'>" + E((M.innings[Lg.inn] && M.innings[Lg.inn].batTeam) || "") + " innings</div>";
        }
        prevInn = Lg.inn;
        if (!pass(Lg)) continue;
        shown++;
        if (Lg.mile) {
          // over summaries read as navy score strips; the human moments keep gold
          rows += /^End of over/i.test(Lg.txt || "")
            ? "<div class='cp-band'>" + E(Lg.txt || "") + "</div>"
            : "<div class='cp-mile'>&#9733; " + E(Lg.txt || "") + "</div>";
        } else {
          var wkR = isW(Lg.out), cls9 = wkR ? " wk" : Lg.out === "4" ? " r4" : Lg.out === "6" ? " r6" : "";
          // "Bowler to Batter :" leads every line - set it like a byline
          var tx9 = String(Lg.txt || ""), who9 = "", m9 = /^(.{2,40}? to .{2,30}?)\s*:\s*(.*)$/.exec(tx9);
          if (m9) { who9 = m9[1]; tx9 = m9[2]; }
          rows += "<div class='db-cr" + cls9 + "'><b>" + E(Lg.no || "") + "</b>" + foDashBall(Lg.out) +
            "<p>" + (who9 ? "<b class='who'>" + E(who9) + "</b> &nbsp;" : "") + E(tx9) + "</p></div>";
        }
      }
      if (!shown) rows += "<div class='db-cr'><p class='k'>Nothing under this filter yet.</p></div>";
      var inn9 = M.innings[M.inns];
      // the header wears the venue painting under a navy veil, broadcast-style
      var hArt = "";
      try { hArt = (document.querySelector("#fo-mstage .fo-mst-bg img") || {}).src || ""; } catch (eHa) {}
      var head = "<div class='cp-head'>" +
        (hArt ? "<img class='cp-art' src='" + hArt + "' alt=''>" : "") + "<span class='cp-veil'></span>" +
        "<div class='cp-hin'><button type='button' id='fo-cp-back'>&#8592; Back to the match</button>" +
        "<div class='cp-title'><b>" + E(M.meta.home) + " v " + E(M.meta.away) + "</b>" +
        "<span>" + E(inn9.batTeam) + " <i>" + inn9.runs + "/" + inn9.wkts + "</i> &middot; " +
        Math.floor(inn9.legal / 6) + "." + (inn9.legal % 6) + " ov" +
        (M.target ? " &middot; need " + Math.max(0, M.target - inn9.runs) : "") + "</span></div>" +
        "<span class='cp-filts'>" + FO_DASH_CFILT.map(function (f9) {
          return "<button type='button' data-cf='" + f9[0] + "' class='" + (filt === f9[0] ? "on" : "") + "'>" + f9[1] + "</button>";
        }).join("") + "</span></div></div>";
      var d = old;
      if (!d) { d = document.createElement("div"); d.id = "fo-commpage"; document.body.appendChild(d); }
      d.setAttribute("data-filt", filt);
      d.innerHTML = head + "<div class='cp-list'><div class='cp-in'>" + rows + "</div></div>";
      d.querySelector(".cp-list").scrollTop = keepScroll;
      d.querySelector("#fo-cp-back").addEventListener("click", function () { d.remove(); });
      d.querySelectorAll("[data-cf]").forEach(function (b9) {
        b9.addEventListener("click", function () { d.setAttribute("data-filt", b9.getAttribute("data-cf")); foDashCommFull(true); });
      });
    } catch (eCf) { try { console.warn("foDashCommFull", eCf); } catch (e0) {} }
  }
  function foDashCss() {
    ...
    s.textContent =
      ...
      // the ui paints ABOVE the docked oval (so the bottom bar always wins),
      // but only its real controls catch the pointer - the oval stays live
      "body.fo-dash .fo-mst-ui{pointer-events:none}" +
      "body.fo-dash .fo-mst-ui .db-card,body.fo-dash .fo-mst-ui .fo-db-bar,body.fo-dash .fo-mst-ui button,body.fo-dash .fo-mst-ui a{pointer-events:auto}" +
      // the painting, contained: same scene layer, clipped to the art panel.
      // Geometry is measured off the grid slot after each paint, not guessed.
      "body.fo-dash .fo-mst-scene{position:absolute;inset:auto;border-radius:14px;overflow:hidden;border:1px solid #e3dccb;z-index:1}" +
      ...
      "body.fo-dash .fo-db-wrap .k{font-family:Manrope,sans-serif;font-weight:500;font-size:10px;letter-spacing:1.6px;text-transform:uppercase;color:#8a7f6e}" +
      ...
      // section voices: burnt-orange card headers, warm gold labels inside
      "body.fo-dash .db-card .hd .k.nv,body.fo-dash #fo-ovhd b{color:#C9571F}" +
      "body.fo-dash .db-cg .k,body.fo-dash .db-p .k,body.fo-dash .fo-db-bug .st .k,body.fo-dash .fo-db-bug .k{color:#b3803f}" +
      "body.fo-dash .db-b{display:inline-flex;min-width:26px;height:26px;padding:0 4px;border-radius:50%;align-items:center;justify-content:center;font:700 13px Manrope,sans-serif;background:#EFE9DB;color:#6b6154;border:1px solid #e0d8c8;font-style:normal}" +
      ...
      "body.fo-dash .fo-db-bug .tm b{display:block;font-family:Manrope,sans-serif;font-weight:600;font-size:15px;letter-spacing:1.3px;text-transform:uppercase}" +
      "body.fo-dash .fo-db-bug .tm b .dot{color:#C9571F;font-size:10px;font-style:normal}" +
      ...
      "body.fo-dash .fo-db-bug .st b.sm{font-size:11px;font-family:Manrope,sans-serif;font-weight:500;max-width:150px}" +
      ...
      "body.fo-dash .db-p .rw .k{white-space:nowrap;max-width:52px;overflow:hidden;text-overflow:ellipsis;font-size:10px}" +
      // the ten stars read exactly as the scorecard reads them: filled gold,
      // half faded, the rest parchment - never all-gold
      "body.fo-dash .db-p .stars{font-size:12.5px;letter-spacing:.5px;white-space:nowrap}" +
      "body.fo-dash .db-p .stars s{text-decoration:none}" +
      "body.fo-dash .db-p .stars em{font-style:normal;color:#ddd5c4}" +
      ...
      "body.fo-dash .db-p .pr u{display:block;text-decoration:none;font-family:Manrope,sans-serif;font-size:10px;letter-spacing:1.2px;text-transform:uppercase;color:#8a7f6e;margin-top:2px}" +
      "body.fo-dash .fo-db-info{display:grid;grid-template-columns:1.45fr 1fr;gap:12px}" +
      "body.fo-dash .fo-db-info>div{min-height:286px}" +
      "body.fo-dash .fo-db-field{border-radius:14px}" +
      "body.fo-dash .db-cg{display:grid;grid-template-columns:1fr 1fr;gap:10px 14px;padding:11px 15px 0}" +
      "body.fo-dash .db-c .v{font-size:12px;font-weight:500;margin-top:1px}" +
      "body.fo-dash .db-gnd{border-top:1px solid #eee7d9;margin:11px 15px 0;padding:9px 0 11px;text-align:center;color:#14243A !important}" +
      "body.fo-dash .fo-db-charts{display:grid;grid-template-columns:1.85fr 1.1fr;gap:12px}" +
      "body.fo-dash .fo-db-charts .db-card{min-height:130px}" +
      "body.fo-dash .db-fw{display:flex;justify-content:space-between;align-items:baseline;padding:6px 15px;border-bottom:1px solid #f0ead9;font-size:12px}" +
      "body.fo-dash .db-fw:last-child{border-bottom:none}" +
      "body.fo-dash .db-fw.pship{border-top:1px solid #eee7d9;border-bottom:none;margin-top:2px}" +
      // the commentary rail
      "body.fo-dash .fo-db-comm{display:flex;flex-direction:column;overflow:hidden}" +
      "body.fo-dash .db-feed{flex:1;overflow:hidden;margin-top:5px}" +
      "body.fo-dash .db-cr{display:grid;grid-template-columns:40px 28px 1fr;gap:9px;align-items:center;padding:8px 14px;border-top:1px solid #f0ead9}" +
      "body.fo-dash .db-cr .db-b{min-width:24px;height:24px;font-size:11px}" +
      "body.fo-dash .db-cr b{font-family:Manrope,sans-serif;font-weight:600;font-size:11.5px;color:#8a7f6e}" +
      "body.fo-dash .db-cr p{margin:0;font-size:12px;line-height:1.45;color:#3a3427}" +
      "body.fo-dash .db-cr .kph{font-family:Manrope,sans-serif;font-weight:600;font-size:10.5px;letter-spacing:1.4px;text-transform:uppercase;color:#C9571F}" +
      "body.fo-dash .db-cr p .who{font-family:Manrope,sans-serif;font-weight:500;font-size:10.5px;letter-spacing:.5px;color:#14243A;text-transform:uppercase}" +
      "body.fo-dash .db-cr.r4{background:#FDF4E7}body.fo-dash .db-cr.r4 b{color:#C9571F}" +
      "body.fo-dash .db-cr.wk{background:#FBEFEA}body.fo-dash .db-cr.wk b,body.fo-dash .db-cr.wk p .who{color:#8E1F13}" +
      "body.fo-dash .db-cr.latest{background:#FBF3E4;border-left:3px solid #C9571F;padding-left:11px;border-top:none}" +
      "body.fo-dash .db-cr.latest b{color:#14243A}" +
      "html body.fo-dash #fo-db-comm-more{margin:10px 16px 14px;padding:10px 0;font-family:Manrope,sans-serif;font-weight:500;font-size:11px;letter-spacing:1.8px;text-transform:uppercase;color:#4c4437 !important;background:#FFFEFC !important;border:1px solid #d9d0bc !important;border-radius:9px;cursor:pointer;box-shadow:none !important}" +
      ...
      "body.fo-dash .fo-db-bar .bt{font-family:Manrope,sans-serif;font-weight:500;font-size:12px;letter-spacing:2px;text-transform:uppercase;color:#FFFEFC}" +
      "body.fo-dash .fo-db-bar .sp{flex:1}" +
      "html body.fo-dash #fo-db-act{background:#C9571F !important;color:#fff !important;font-family:Manrope,sans-serif;font-weight:600;font-size:12px;letter-spacing:1.8px;text-transform:uppercase;border:none !important;border-radius:9px;padding:12px 26px;cursor:pointer;box-shadow:none !important}" +
      "html body.fo-dash #fo-db-act:hover{filter:brightness(1.08)}" +
      "html body.fo-dash .fo-db-bar .fo-mst-next{position:static !important;width:auto !important;min-height:0 !important;padding:12px 26px !important;font-size:12px !important;letter-spacing:1.8px !important;background:linear-gradient(180deg,#E8B96A,#8F6A1C) !important;color:#0C1B2E !important;border:none !important;border-radius:9px !important;box-shadow:none !important;transform:none !important}" +
      ...
      // the green is the point: the pitch map fills the card, the chrome yields
      "body.fo-dash #fo-oval .ov-svg{flex:1;min-height:0;transform:scale(1.13);transform-origin:50% 46%}" +
      "body.fo-dash #fo-oval .ov-ctl,body.fo-dash #fo-oval .ov-note{display:none !important}" +
      ...
      // the scorecard drawer wears daylight and takes the WHOLE page
      "html body.fo-dash.fo-thd #page .ftp-match-shell{background:#F6F3EB !important;border:none !important;border-radius:0 !important;box-shadow:none !important;left:0;right:0;top:52px;bottom:0;width:auto;padding:16px max(24px,calc(50vw - 610px)) 40px}" +
      ...
      // the full commentary page: broadcast header over the venue painting,
      // then the whole book - boundaries glow, wickets bleed, overs bar navy
      "#fo-commpage{position:fixed;inset:0;z-index:900;background:#F6F3EB;display:flex;flex-direction:column;font-family:Manrope,system-ui,sans-serif;color:#14243A}" +
      "#fo-commpage .cp-head{position:relative;overflow:hidden;background:#14243A;flex:0 0 auto}" +
      ...
      "#fo-commpage .cp-title span{font:500 11px Manrope,sans-serif;letter-spacing:1.8px;text-transform:uppercase;color:#c9d0da}" +
      "#fo-commpage .cp-title span i{font-style:normal;color:#E8B96A;font-size:13px}" +
      "html body #fo-cp-back{font-family:Manrope,sans-serif;font-weight:500;font-size:11px;letter-spacing:1.6px;text-transform:uppercase;color:#FFFEFC !important;background:rgba(255,255,255,.1) !important;border:1px solid rgba(255,255,255,.35) !important;border-radius:9px;padding:9px 14px;cursor:pointer;box-shadow:none !important}" +
      "html body #fo-cp-back:hover{border-color:#E8B96A !important;color:#E8B96A !important}" +
      "#fo-commpage .cp-filts{margin-left:auto;display:flex;gap:6px}" +
      "html body #fo-commpage .cp-filts button{font-family:Manrope,sans-serif;font-weight:500;font-size:10.5px;letter-spacing:1.4px;text-transform:uppercase;color:#e6e0d2 !important;background:rgba(255,255,255,.08) !important;border:1px solid rgba(255,255,255,.3) !important;border-radius:999px;padding:7px 14px;cursor:pointer;box-shadow:none !important}" +
      "html body #fo-commpage .cp-filts button.on{background:#C9571F !important;border-color:#C9571F !important;color:#fff !important}" +
      "#fo-commpage .cp-list{flex:1;overflow:auto}" +
      "#fo-commpage .cp-in{max-width:1120px;margin:0 auto;padding:16px 24px 40px}" +
      "#fo-commpage .db-cr{display:grid;grid-template-columns:46px 30px 1fr;gap:10px;align-items:center;padding:10px 14px;border-top:1px solid #eee7d9;background:#FFFEFC}" +
      "#fo-commpage .db-cr:first-child{border-top:none}" +
      "#fo-commpage .db-cr b{font-family:Manrope,sans-serif;font-weight:600;font-size:12px;color:#8a7f6e}" +
      "#fo-commpage .db-cr p{margin:0;font-size:13.5px;line-height:1.55;color:#3a3427}" +
      "#fo-commpage .db-cr p .who{font-family:Manrope,sans-serif;font-weight:500;font-size:12px;letter-spacing:.6px;color:#14243A;text-transform:uppercase}" +
      "#fo-commpage .db-cr.r4{background:#FDF4E7;border-left:3px solid #C9571F;padding-left:11px}" +
      "#fo-commpage .db-cr.r4 b{color:#C9571F}" +
      "#fo-commpage .db-cr.r6{background:#F1EFE7;border-left:3px solid #14243A;padding-left:11px}" +
      "#fo-commpage .db-cr.r6 p .who{color:#8a6a1f}" +
      "#fo-commpage .db-cr.wk{background:#FBEFEA;border-left:3px solid #8E1F13;padding-left:11px}" +
      "#fo-commpage .db-cr.wk b,#fo-commpage .db-cr.wk p .who{color:#8E1F13}" +
      "#fo-commpage .db-cr.wk p{font-weight:500}" +
      "#fo-commpage .cp-band{display:flex;align-items:center;gap:12px;background:#14243A;color:#E8B96A;border-radius:9px;padding:9px 16px;margin:10px 0;font:500 11.5px Manrope,sans-serif;letter-spacing:1.6px;text-transform:uppercase}" +
      "#fo-commpage .cp-band.inn{background:#C9571F;color:#fff;font-size:13px}" +
      "#fo-commpage .cp-mile{background:linear-gradient(90deg,#FBF3E4,#FDFAF2);border:1px solid #E8B96A;border-radius:9px;padding:10px 16px;margin:8px 0;font:600 13px Manrope,sans-serif;color:#7a5a17}" +
      "#fo-commpage .k{font-family:Manrope,sans-serif;font-weight:500;font-size:10px;letter-spacing:1.6px;text-transform:uppercase;color:#8a7f6e}" +
      ...
      "body.fo-dash .fo-db-bug .bb9 .db-b{min-width:17px;height:17px;font-size:10px}" +
  ...
  function foThCss() {
    ...
    s.textContent =
      ...
      "html body.fo-th #fo-mstage .fo-mst-next{font-family:Manrope,sans-serif !important;font-weight:600 !important;letter-spacing:1.8px;text-transform:uppercase;font-size:12.5px;background:linear-gradient(180deg,#E8B96A,#8F6A1C) !important;color:#0C1B2E !important;border:none !important;border-radius:999px;padding:11px 22px;cursor:pointer;box-shadow:0 4px 0 rgba(16,27,45,.35),0 8px 22px rgba(201,162,75,.3) !important}" +
      ...
      "html body.fo-th #fo-mst-spd button{flex:1;min-height:48px;min-width:64px;font-family:Manrope,sans-serif !important;font-weight:600 !important;font-size:15px;letter-spacing:1.5px;background:rgba(5,20,40,.72) !important;border:1.5px solid rgba(255,255,255,.3) !important;color:rgba(255,255,255,.85) !important;border-radius:12px;cursor:pointer;backdrop-filter:blur(8px);padding:10px 0;box-shadow:none !important}" +
      "html body.fo-th #fo-mst-spd button.on{background:linear-gradient(180deg,#E8B96A,#8F6A1C) !important;border-color:#E8B96A !important;color:#0C1B2E !important;box-shadow:0 4px 0 rgba(16,27,45,.35),0 8px 22px rgba(201,162,75,.3) !important}" +
      ...
      "html body.fo-th .fo-mst-ask .ask-opts button:hover{border-color:#E8B96A !important;background:rgba(240,185,78,.12) !important}" +
      ...
      "body.fo-th .fo-mst-score .tm{display:inline-block;font-family:Manrope,sans-serif;font-weight:600;font-size:15px;letter-spacing:1px;color:#fff;margin-right:9px;vertical-align:baseline}" +
      "body.fo-th .fo-mst-score b{display:inline-block;font-size:30px;vertical-align:baseline}" +
      "body.fo-th .fo-mst-score .ovs{display:block;font-family:Manrope,sans-serif;font-size:11px;letter-spacing:2.4px;text-transform:uppercase;color:#E8B96A;margin-top:3px}" +
      ...
      "body.fo-th .fo-mst-moment .rib .chip{border:none;background:none;padding:0;color:#E8B96A;backdrop-filter:none;font-size:12.5px}" +
      "body.fo-th .fo-mst-moment .rib .kph{font-family:Manrope,sans-serif;font-size:12.5px;font-weight:600;letter-spacing:1.8px;text-transform:uppercase;color:#22D3E0;white-space:nowrap}" +
      "body.fo-th .fo-mst-moment .rib p{margin:0;font-family:Manrope,sans-serif;font-size:12px;font-weight:500;letter-spacing:.9px;text-transform:uppercase;color:rgba(255,255,255,.96);white-space:normal;overflow:visible;text-overflow:clip;max-width:none;text-align:center;line-height:1.35}" +
      "body.fo-th .fo-mst-score .mbs{display:flex;gap:5px;justify-content:center;margin-top:7px;min-height:20px}" +
      "body.fo-th .fo-mst-score .mb{width:20px;height:20px;display:flex;align-items:center;justify-content:center;border-radius:50%;font-family:Manrope,sans-serif;font-weight:600;font-size:10px;color:#fff;background:rgba(255,255,255,.13);border:1.5px solid rgba(255,255,255,.24)}" +
      ...
      "body.fo-th #fo-th-gauge .gv b{font-family:Manrope,sans-serif;font-weight:600;font-size:19px;letter-spacing:1px}" +
      "body.fo-th #fo-th-gauge .gv i{font-style:normal;font-family:Manrope,sans-serif;font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#E8B96A;margin-left:4px}" +
      ...
      "#fo-th-rail button:hover{border-color:#E8B96A;color:#E8B96A}" +
      "#fo-th-rail button.on{border-color:#E8B96A;color:#E8B96A;box-shadow:0 0 0 3px rgba(240,185,78,.25)}" +
      "#fo-th-rail button.ping::after{content:'';position:absolute;top:3px;right:3px;width:9px;height:9px;border-radius:50%;background:#C9571F;border:1.5px solid rgba(5,20,40,.9)}" +
      ...
      "html body.fo-th.fo-thd #page .ftp-match-links a.on{background:#E8B96A !important;color:#0C1B2E !important}" +
      ...
      "#fo-ovhd b{font-family:Manrope,sans-serif;font-weight:600;font-size:11px;letter-spacing:2.4px;text-transform:uppercase;color:#E8B96A}" +
      "#fo-ovhd span{flex:1;font-family:Manrope,sans-serif;font-size:10px;letter-spacing:1.6px;text-transform:uppercase;color:rgba(255,255,255,.7);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}" +
      ...
      "#fo-ovhd button:hover{border-color:#E8B96A;color:#E8B96A}" +
      "#fo-ovhd #fo-ovpin.on{color:#E8B96A;border-color:#E8B96A}" +
      ...
      "#fo-th-gf b{display:block;font-family:Manrope,sans-serif;font-size:10px;font-weight:600;letter-spacing:2px;text-transform:uppercase;color:#E8B96A;margin-bottom:2px}" +
      ...
      "#fo-th-cut .ct b{font-family:Manrope,sans-serif;font-weight:600;font-size:30px;letter-spacing:5px;color:#E8B96A}" +
      "#fo-th-cut .ct i{font-style:normal;font-family:Manrope,sans-serif;font-size:13px;letter-spacing:2px;text-transform:uppercase}" +
      ...
      "html body #page #fo-mst-talk .tb.tbc,html body.ftpskin #page #fo-mst-talk .tb.tbc{margin-left:auto;color:#E8B96A !important;border-color:rgba(243,211,122,.55) !important}" +
      ...
      "#fo-mob-sheet,#fo-mob-nav,#fo-mob-flash,#fo-mob-spdchip{display:none}" +
      ...
      "#fo-mstage::after{content:'';position:absolute;left:0;right:0;bottom:0;z-index:6;height:calc(124px + env(safe-area-inset-bottom));background:rgba(7,17,33,.9);border:1px solid rgba(255,255,255,.14);border-bottom:none;border-radius:20px 20px 0 0;backdrop-filter:blur(14px);box-shadow:0 -14px 40px rgba(0,0,0,.5);transition:height .28s ease;pointer-events:none}" +
      "body.fo-th[data-mobsheet='oval'] #fo-mstage::after{height:calc(100dvh - 126px - env(safe-area-inset-top))}" +
      "body.fo-th[data-mobsheet='score'] #fo-mstage::after{height:calc(88dvh + env(safe-area-inset-bottom))}" +
      ...
      "body.fo-th .fo-mst-score .tm{font-size:11.5px;letter-spacing:1.6px;text-transform:uppercase;color:#E8B96A;margin:0}" +
      ...
      "body.fo-th .fo-mst-score i{display:none}" +
      "body.fo-th .fo-mst-score i.chase{display:block;flex-basis:100%;text-align:center;font-size:10px;letter-spacing:.6px;color:rgba(255,255,255,.78);margin-top:1px}" +
      // the last six deliveries ride under the bug
      "body.fo-th .fo-mst-score .mbs{display:flex;flex-basis:100%;justify-content:center;gap:5px;margin-top:4px;min-height:0}" +
      "body.fo-th .fo-mst-score .mb{width:21px;height:21px;font-size:10.5px;border-width:1px}" +
      ...
      "body.fo-th .fo-mst-wx span{background:none !important;border:none !important;padding:0 !important;margin:0 !important;font-size:10px;letter-spacing:1px;white-space:nowrap;color:#F4EFE3}" +
      "body.fo-th .fo-mst-wx span+span::before{content:'\\2022';margin:0 6px;color:#E8B96A}" +
      ...
      "body.fo-th .fo-mst-p{position:fixed;top:auto !important;bottom:calc(138px + env(safe-area-inset-bottom)) !important;height:auto !important;min-height:64px;max-width:none;padding:6px 10px;gap:9px;border-radius:0;background:rgba(7,18,36,.8);border:1px solid rgba(255,255,255,.16);backdrop-filter:blur(8px);box-shadow:0 8px 22px rgba(0,0,0,.4);z-index:50}" +
      ...
      // the role line carries the matchup facts: hand for the batter, the
      // bowler's type for the bowler
      "body.fo-th .fo-mst-p .rl{display:block;font-size:10px;letter-spacing:1.1px;text-transform:uppercase;color:#E8B96A;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:1px}" +
      ...
      "body.fo-th .fo-mst-p .stars{display:block;font-size:10px;letter-spacing:1px;line-height:1.2;margin-top:1px;white-space:nowrap}" +
      "body.fo-th[data-mobsheet='score'] .fo-mst-p{display:none !important}" +
      "body.fo-th[data-mobsheet='score'] .fo-mst-wx{display:none !important}" +
      "body.fo-th[data-mobsheet='oval'] .fo-mst-p{top:calc(158px + env(safe-area-inset-top)) !important;bottom:auto !important;z-index:53;background:transparent;border-color:transparent;backdrop-filter:none;box-shadow:none}" +
      ...
      "#fo-mob-sheet{display:block;position:fixed;left:0;right:0;bottom:0;z-index:51;background:none;border:none;box-shadow:none;height:calc(124px + env(safe-area-inset-bottom));transition:height .28s ease;touch-action:none;pointer-events:none}" +
      "body.fo-th[data-mobsheet='live'] #fo-mstage::after{background:linear-gradient(180deg,rgba(4,10,20,0),rgba(4,10,20,.52) 34%,rgba(4,10,20,.84) 78%,rgba(4,10,20,.9));border:none;border-radius:0;box-shadow:none;backdrop-filter:none;height:calc(178px + env(safe-area-inset-bottom))}" +
      ...
      "body.fo-th.fo-spd-open[data-mobsheet='live'] #fo-mob-sheet,body.fo-th.fo-spd-open[data-mobsheet='live'] #fo-mstage::after{height:calc(216px + env(safe-area-inset-bottom))}" +
      "body.fo-th[data-mobsheet='oval'] #fo-mob-sheet{height:calc(100dvh - 126px - env(safe-area-inset-top))}" +
      "body.fo-th[data-mobsheet='score'] #fo-mob-sheet{height:calc(88dvh + env(safe-area-inset-bottom))}" +
      "@media (prefers-reduced-motion:reduce){#fo-mob-sheet{transition:none}}" +
      // tab bar: LIVE / OVAL / SCORE
      ...
      "html body #fo-mob-nav button span{font-family:Manrope,sans-serif;font-size:10px;font-weight:600;letter-spacing:2px;text-transform:uppercase}" +
      "html body #fo-mob-nav button.on{color:#E8B96A}" +
      "html body #fo-mob-nav button.on span{border-bottom:2px solid #E8B96A;padding-bottom:1px}" +
      // latest delivery: chip + line, result as a small badge on the right
      "body.fo-th .fo-mst-moment{top:auto;left:14px;right:66px;transform:none;bottom:calc(70px + env(safe-area-inset-bottom));width:auto;background:none;border:none;padding:0;backdrop-filter:none;display:flex;flex-direction:row-reverse;align-items:center;gap:8px;z-index:53;transition:bottom .25s ease}" +
      "body.fo-th.fo-spd-open[data-mobsheet='live'] .fo-mst-moment{bottom:calc(112px + env(safe-area-inset-bottom))}" +
      "body.fo-th:has(#fo-mst-done) .fo-mst-moment{bottom:calc(126px + env(safe-area-inset-bottom))}" +
      ...
      "body.fo-th .fo-mst-moment .rib p{font-size:12.5px;letter-spacing:.2px;text-transform:none;text-align:left;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#F4EFE3;text-shadow:0 1px 6px rgba(0,0,0,.7)}" +
      "body.fo-th .fo-mst-moment .rib .kph{display:none}" +
      "body.fo-th[data-mobsheet='score'] .fo-mst-moment{display:none !important}" +
      ...
      "body.fo-th #fo-mst-spd{display:none;position:fixed;left:50%;right:auto;top:auto;transform:translateX(-50%);width:auto;gap:7px;z-index:57;bottom:calc(62px + env(safe-area-inset-bottom))}" +
      "body.fo-th.fo-spd-open #fo-mst-spd{display:flex}" +
      "html body.fo-th #fo-mst-spd button{flex:0 0 auto;min-width:58px;min-height:34px;font-size:12px;letter-spacing:1px;padding:7px 0;border-radius:9px}" +
      "html body #fo-mob-spdchip{display:flex;align-items:center;justify-content:center;position:fixed;right:12px;bottom:calc(66px + env(safe-area-inset-bottom));z-index:57;min-width:44px;min-height:32px;padding:5px 10px;border-radius:999px;font-family:Manrope,sans-serif !important;font-weight:600 !important;font-size:12px;letter-spacing:1px;background:rgba(5,20,40,.6) !important;border:1.5px solid rgba(255,255,255,.3) !important;color:rgba(255,255,255,.88) !important;cursor:pointer;box-shadow:none !important;transition:bottom .25s ease}" +
      "body.fo-spd-open #fo-mob-spdchip{border-color:#E8B96A !important;color:#E8B96A !important}" +
      "body.fo-th[data-mobsheet='score'] #fo-mob-spdchip,body.fo-th.fo-thd:not([data-mobsheet='oval']) #fo-mob-spdchip{display:none}" +
      // in the oval sheet the bottom edge belongs to the tactics row - the
      // chip and the tempo row step up beside the delivery line
      "body.fo-th[data-mobsheet='oval'] #fo-mob-spdchip{bottom:auto;top:calc(226px + env(safe-area-inset-top));transition:none}" +
      "body.fo-th[data-mobsheet='oval'] #fo-mst-spd{bottom:auto;top:calc(266px + env(safe-area-inset-top))}" +
      "body.fo-th:has(#fo-mst-done) #fo-mob-spdchip{display:none}" +
      ...
      "html body.fo-th[data-mobsheet='oval'] #fo-oval{display:block !important;position:fixed !important;left:6px !important;right:6px !important;top:calc(258px + env(safe-area-inset-top)) !important;bottom:calc(170px + env(safe-area-inset-bottom)) !important;transform:none;width:auto !important;max-width:none !important;max-height:none;z-index:54;background:transparent;border:none;border-radius:0 !important;box-shadow:none;backdrop-filter:none;overflow:visible}" +
      // .ov-svg IS the <svg>: fill the pane and let the viewBox centre itself
      "body.fo-th[data-mobsheet='oval'] #fo-oval .ov-svg{width:100%;height:100% !important;max-height:none;background:transparent}" +
      "body.fo-th[data-mobsheet='oval'] #fo-oval .ov-snd{position:absolute;top:4px;right:6px;z-index:2}" +
      // the big centre-screen ball result flash
      "#fo-mob-flash{display:block;position:fixed;left:0;right:0;top:40%;z-index:56;text-align:center;font-family:Manrope,sans-serif;font-weight:600;font-size:62px;letter-spacing:7px;color:#F4EFE3;text-shadow:0 4px 26px rgba(0,0,0,.9),0 0 70px rgba(0,0,0,.55);pointer-events:none;animation:foMobFlash 1.5s ease forwards}" +
      "#fo-mob-flash.k-boundary{color:#f9c957}#fo-mob-flash.k-wicket{color:#ff6c61;letter-spacing:9px}#fo-mob-flash.k-dot{color:#c7e4e8;font-size:52px}" +
      "@keyframes foMobFlash{0%{opacity:0;transform:scale(.72)}14%{opacity:1;transform:scale(1)}72%{opacity:1}100%{opacity:0;transform:scale(1.05)}}" +
      "@media (prefers-reduced-motion:reduce){#fo-mob-flash{animation:none}}" +
      "body.fo-th #fo-ovhd{display:none}" +
      "body.fo-th #fo-oval .ov-note{display:none}" +
      "body.fo-th #fo-ovhd button{display:none}" +
      // tactics live IN the oval now: field call when bowling, batting intent
      // when batting, pinned above the tab bar with the active choice lit
      ...
      "html body.fo-th[data-mobsheet='oval'] #fo-oval #fo-mst-talk .tl{flex-basis:100%;text-align:center;font-family:Manrope,sans-serif;font-size:10px;font-weight:600;letter-spacing:2.2px;text-transform:uppercase;color:#E8B96A;margin:0}" +
      ...
      "html body.fo-th[data-mobsheet='oval'] #fo-oval #fo-mst-talk .tb.on{background:#E8B96A !important;border-color:#E8B96A !important;color:#0C1B2E !important}" +
      "html body.fo-th[data-mobsheet='oval'] #fo-oval #fo-mst-talk .tb.tbc{margin-left:0;color:#E8B96A !important;border-color:rgba(243,211,122,.55) !important;background:rgba(243,211,122,.1) !important}" +
      ...
      // everything else lives elsewhere on the phone (or not at all)
      "html body.fo-th.fo-thd #page .ftp-match-links a[onclick*=\"'Details'\"],html body.fo-th.fo-thd #page .ftp-match-links a[onclick*=\"'Orders'\"],html body.fo-th.fo-thd #page .ftp-match-links a[onclick*=\"'Match Ratings'\"],html body.fo-th.fo-thd #page .ftp-match-links a[onclick*=\"'Worm'\"],html body.fo-th.fo-thd #page .ftp-match-links a[onclick*=\"'Partnerships'\"],html body.fo-th.fo-thd #page .ftp-match-links a[onclick*=\"'Rivalry'\"],html body.fo-th.fo-thd #page .ftp-match-links a[data-fo-lineups]{display:none !important}" +
  ...
  function foMatchStage() {
    try {
        ...
        if (kphTx && bw && !M.done) {
          ...
          if (!gEl) {
            ...
            gEl.innerHTML = "<svg viewBox='0 0 100 58'>" +
              "<defs><linearGradient id='foGg' gradientUnits='userSpaceOnUse' x1='10' y1='50' x2='90' y2='50'><stop offset='0' stop-color='#22D3E0'/><stop offset='.55' stop-color='#E8B96A'/><stop offset='1' stop-color='#E0504B'/></linearGradient></defs>" +
      ...
      // the last six deliveries, as beads - a rolling window, so the row
      // never goes empty at the top of an over
      var cur = [];
      for (var i2 = 0; i2 < M.log.length && cur.length < 6; i2++) {
      ...
      try {
        if (foMobIsOn() && !foDashOn() && L && !M.done && window.__foMobFlashN !== M.log.length) {
          ...
          var oF = L.out;
          var flashTx = oF === "dot" ? "DOT" : oF === "1" ? "ONE" : oF === "2" ? "TWO" : oF === "3" ? "THREE" :
      ...
      if (foDashOn()) {
        try {
          ...
          var place9 = function (node, slot) {
            if (!node || !slot) return;
            var r9 = slot.getBoundingClientRect(), b9 = el.getBoundingClientRect();
            // inline-important: the theatre's own !important geometry must lose
            node.style.setProperty("top", (r9.top - b9.top + el.scrollTop).toFixed(0) + "px", "important");
            node.style.setProperty("left", (r9.left - b9.left + el.scrollLeft).toFixed(0) + "px", "important");
            node.style.setProperty("width", r9.width.toFixed(0) + "px", "important");
            node.style.setProperty("height", r9.height.toFixed(0) + "px", "important");
          };
          place9(el.querySelector(".fo-mst-scene"), el.querySelector(".fo-db-art"));
          place9(ovD, el.querySelector(".fo-db-field"));
          ...
          if (act9) act9.addEventListener("click", function () {
            try {
              ...
              if (typeof UI !== "undefined") { UI.matchTab = "Scorecard"; document.body.setAttribute("data-thtab", "Scorecard"); }
  ...
  try {
    ...
    msCss.textContent =
      ...
      ".fo-mst-wx span{padding:4px 8px;color:rgba(255,255,255,.85);font-family:Manrope,sans-serif;font-size:10px;font-weight:600;letter-spacing:1.2px;text-transform:uppercase;white-space:nowrap}" +
      ".fo-mst-ctlg{display:inline-flex;gap:8px;align-items:center}" +
      "html body #page .fo-mst-next,html body.ftpskin #page .fo-mst-next{font-family:Manrope,sans-serif !important;font-weight:600 !important;letter-spacing:1.8px;text-transform:uppercase;font-size:12.5px;background:linear-gradient(180deg,#E8B96A,#8F6A1C) !important;color:#0C1B2E !important;border:none !important;border-radius:999px;padding:11px 22px;cursor:pointer;box-shadow:0 4px 0 rgba(16,27,45,.35),0 8px 22px rgba(201,162,75,.3) !important}" +
      ...
      "html body #page .fo-mst-autob,html body.ftpskin #page .fo-mst-autob{font-family:Manrope,sans-serif !important;font-weight:600 !important;letter-spacing:1.5px;text-transform:uppercase;font-size:10.5px;background:rgba(5,22,43,.72) !important;color:rgba(255,255,255,.8) !important;border:1.5px solid rgba(255,255,255,.3) !important;border-radius:999px;padding:10px 16px;cursor:pointer;backdrop-filter:blur(8px);box-shadow:none !important}" +
      ...
      ".fo-mst-ask .askopt .who b{display:block;font-family:Manrope,sans-serif;font-weight:600;font-size:13.5px;letter-spacing:.5px;max-width:180px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}" +
      ".fo-mst-ask .askopt .who i{font-style:normal;font-family:Manrope,sans-serif;font-size:10px;letter-spacing:1.4px;text-transform:uppercase;color:rgba(255,255,255,.72)}" +
      ".fo-mst-ask .askopt:hover{transform:translateY(-5px);border-color:#E8B96A;box-shadow:0 14px 34px rgba(0,0,0,.5)}" +
      ...
      ".fo-mst-ask .askopt .tag{position:static;transform:none;font-family:Manrope,sans-serif;font-size:10px;font-weight:600;letter-spacing:1.8px;text-transform:uppercase;color:#E8B96A;white-space:nowrap}" +
      ...
      ".fo-mst-ask .ask-t{font-family:Manrope,sans-serif;font-weight:600;font-size:19px;letter-spacing:1.6px;text-transform:uppercase;color:#E8B96A}" +
      ...
      "html body #page .fo-mst-ask .ask-opts button:hover{border-color:#E8B96A !important;background:rgba(240,185,78,.12) !important}" +
      ...
      ".fo-mst-ask .ask-opts button i{font-style:normal;font-family:Manrope,sans-serif;font-size:10px;letter-spacing:1.6px;text-transform:uppercase;color:#E8B96A}" +
      ".fo-mst-ask .ask-bar{height:4px;border-radius:99px;background:rgba(255,255,255,.14);margin-top:13px;overflow:hidden}" +
      ".fo-mst-ask .ask-bar i{display:block;height:100%;background:linear-gradient(90deg,#E8B96A,#8F6A1C);animation:foMstAskT linear forwards}" +
      ...
      "#fo-mst-talk .tl{font-family:Manrope,sans-serif;font-size:10px;font-weight:600;letter-spacing:2px;text-transform:uppercase;color:#8ea0c0;margin-right:4px}" +
      ...
      "html body #page #fo-mst-talk .tb.on,html body.ftpskin #page #fo-mst-talk .tb.on{color:#0C1B2E !important;background:linear-gradient(180deg,#E8B96A,#8F6A1C) !important;border-color:#E8B96A !important}" +
      ...
      ".fo-mst-score b{display:block;font-family:Manrope,sans-serif;font-weight:600;font-size:44px;line-height:1;letter-spacing:1px}" +
      ".fo-mst-score span{display:block;font-size:12px;color:rgba(255,255,255,.85);margin-top:2px}" +
      ".fo-mst-score i{display:block;font-style:normal;font-family:Manrope,sans-serif;font-size:10px;letter-spacing:1.6px;text-transform:uppercase;color:#E8B96A;margin-top:3px}" +
      ...
      ".fo-mst-p .rl{color:#f5c85b;font-family:Manrope,sans-serif;font-size:10px;font-weight:600;letter-spacing:1.8px;text-transform:uppercase}" +
      ".fo-mst-p .nm{font-family:Manrope,sans-serif;font-weight:600;font-size:16.5px;letter-spacing:.4px;line-height:1.12;margin:2px 0 2px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}" +
      ...
      ".fo-mst-p.pbat .stars em.f{color:#E8B96A}" +
      ".fo-mst-p.pbat .stars em.h{background:linear-gradient(90deg,#E8B96A 50%,rgba(255,255,255,.28) 50%);-webkit-background-clip:text;background-clip:text;color:transparent}" +
      ...
      ".fo-mst-moment .chip{display:inline-flex;padding:5px 11px;border:1px solid rgba(255,255,255,.35);border-radius:999px;background:rgba(7,25,48,.66);backdrop-filter:blur(8px);color:rgba(255,255,255,.85);font-family:Manrope,sans-serif;font-size:10px;font-weight:600;letter-spacing:1.8px;text-transform:uppercase}" +
      ".fo-mst-moment .t{margin:10px 0 7px;font-family:Manrope,sans-serif;font-weight:600;font-size:clamp(48px,6.5vw,92px);line-height:.9;letter-spacing:2px;text-transform:uppercase}" +
      ...
      ".fo-mst-foot .gf b{color:#f7c75b;font-family:Manrope,sans-serif;font-size:10px;font-weight:600;letter-spacing:1.6px;text-transform:uppercase;margin-right:4px}" +
      ...
      "html body #page #fo-commfull-btn,html body.ftpskin #page #fo-commfull-btn{display:block;margin:8px auto 2px;font-family:Manrope,sans-serif !important;font-weight:600 !important;font-size:10.5px;letter-spacing:1.8px;text-transform:uppercase;color:#6A6354 !important;background:transparent !important;border:1.5px solid #d8d2c5 !important;border-radius:999px;padding:8px 18px;cursor:pointer;box-shadow:none !important}" +
      ...
      "#fo-commfull .cf-hd{display:flex;justify-content:space-between;align-items:center;background:#07162E;color:#FFFEFC;padding:12px 16px;font-family:Manrope,sans-serif;font-size:12px;font-weight:600;letter-spacing:2px;text-transform:uppercase}" +
      ...
      ".fo-mst-wx span{font-size:10px;padding:3px 6px}" +
      "html body #page .fo-mst-next{font-size:11px;padding:10px 16px}" +
      "html body #page .fo-mst-autob{font-size:10px;padding:9px 12px}" +
