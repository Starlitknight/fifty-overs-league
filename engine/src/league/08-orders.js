  ...
  var FO_ORD_PLANS = [
    { id: "attack", ic: "&#9889;", nm: "Attack early", sub: "Take their new ball on.", pi: { pp: 1, mid: 0, death: 2 }, fp: { pp: "att", mid: "bal", death: "def" } },
    { id: "build", ic: "&#128737;&#65039;", nm: "Build, then explode", sub: "Wickets in hand, launch late.", pi: { pp: -1, mid: 0, death: 2 }, fp: { pp: "bal", mid: "bal", death: "def" } },
    { id: "squeeze", ic: "&#128376;&#65039;", nm: "Squeeze them out", sub: "Bat safe, strangle with the ball.", pi: { pp: 0, mid: 0, death: 1 }, fp: { pp: "bal", mid: "def", death: "def" } }
  ];
  ...
  // class-named elements only: the chips style bare <i>/<u> tags for badges,
  // and a bar built from those tags inherits the badge look
  function foOrdB3(items) {
    return "<span class='b3'>" + items.map(function (it) {
      var v = Math.max(0, Math.min(100, Math.round(it[2] || 0)));
      return "<span class='b3i' title='" + E(it[1]) + ": " + v + "'><span class='b3t'><b style='height:" + Math.max(10, v) + "%;background:" + foOrdVCol(v) + "'></b></span><span class='b3l'>" + it[0] + "</span></span>";
    }).join("") + "</span>";
  }
  ...
  function foOrdPlanVisual() {
    try {
      ...
      var chip = function (nm, i, dim) {
        ...
        // the batting-order lens: technique (vs pace + vs spin), power, stamina
        var bars = foOrdB3([
          ["Tec", "Technique (vs pace & spin)", ((sk.vsPace || 0) + (sk.vsSpin || 0)) / 2],
          ["Pow", "Power hitting", sk.power || 0],
          ["Sta", "Stamina", sk.stamina || 0]
        ]);
        ...
        return "<button type='button' class='xc xc-" + role + (dim ? " xc-dim" : "") + "' data-fo-pc='" + E(nm) + "'>" +
          ...
          "<span class='r2'>" + foOrdStarHTML(foOrdStars(p, ranked)) + bars + "</span>" +
      ...
      var mix = "<div class='fo-ord-xinote'><b>" + nSeam + " seam</b> · <b>" + nSpin + " spin</b> · <b>" + nAr + " all-rounder" + (nAr === 1 ? "" : "s") + "</b> · tap a player for his card</div>";
      ...
      var bench = benchNames.length
        ? "<div class='fo-ord-vzh'>Bench</div><div class='fo-ord-xis'>" + benchNames.map(function (nm) { return chip(nm, null, true); }).join("") + "</div>"
        : "";
      ...
      var phases = "<div class='fo-ord-ph3'>" + ["pp", "mid", "death"].map(function (k, i) {
        return "<div><b>" + ["Powerplay 1-10", "Middle 11-40", "Death 41-50"][i] + "</b><span>" + W[String(pi[k] || 0)] + "</span><span class='f'>" + (F[fp[k]] || "Balanced") + " field</span></div>";
      }).join("") + "</div>";
      // one lane per bowler: filled blocks are his overs, powerplay and death
      // zones lightly shaded - who bowls when, in one ink colour
      var g = App.orders.grid || {}, tot = {}, first = {};
      for (var o = 1; o <= 50; o++) {
        var nm2 = g[o];
        if (nm2) { tot[nm2] = (tot[nm2] || 0) + 1; if (first[nm2] == null) first[nm2] = o; }
      }
      var bowlNames = Object.keys(tot).sort(function (a, b) { return first[a] - first[b]; });
      var lanes = "<div class='fo-ord-lanes'>" + bowlNames.map(function (nmL) {
        var cellsL = "";
        for (var oL = 1; oL <= 50; oL++)
          cellsL += "<i class='" + (g[oL] === nmL ? "f" : "") + (oL <= 10 ? " pp" : oL >= 41 ? " dth" : "") + "' title='Over " + oL + "'></i>";
        return "<div class='fo-ord-lane'><span class='ln'>" + E(dispNm(nmL)) + "</span><span class='lt'>" + cellsL + "</span><u>" + tot[nmL] + "</u></div>";
      }).join("") +
      ...
      // each bowler as a small card: type, overs, the bowling lens and talents
      var legend = "<div class='fo-ord-bws'>" + bowlNames.map(function (nm3) {
        var p3 = by[nm3] || {}, sk3 = p3.skills || {};
        var bars3 = foOrdB3([
          ["Wkt", "Wicket threat", sk3.wicket || 0],
          ["Eco", "Economy (hard to score off)", sk3.economy || 0],
          ["Sta", "Stamina", sk3.stamina || 0]
        ]);
        return "<button type='button' class='bw' data-fo-pc='" + E(nm3) + "'>" +
          ...
          "<span class='bw-r'>" + bars3 + (foOrdTalPills(p3, 2) || "") + "</span></button>";
      }).join("") + "</div>";
  ...
  function foOrdersUI() {
    ...
    if (foOrdMode() === "simple") {
      ...
      page.innerHTML = "<div class='crumb'>" + E(opp.home) + " v " + E(opp.away) + " &raquo; Orders</div>" + cond +
        ...
        "<span class='bx'><span class='sp'>The Gaffer</span><span class='tx'>&ldquo;My plan for these conditions, boss. Pick the game plan - the rest is done.&rdquo;</span></span></div>" +
        foOrdPlanVisual() +
        ...
        "<button data-fo-act='reroll'>New plan</button>" +
        "<button data-fo-act='fine'>Fine-tune</button>" +
  ...
  try {
    ...
    foOrdCss.textContent =
      ...
      ".fo-ord-xis{display:grid;grid-template-columns:repeat(auto-fill,minmax(158px,1fr));gap:6px}" +
      ...
      ".fo-ord-lane .lt i{flex:1;min-width:0;background:rgba(28,36,51,.05);border-radius:1px}" +
      ...
      "@media(max-width:480px){.fo-ord-lane .ln{flex-basis:62px;font-size:9.5px}.fo-ord-lane.lax em{font-size:6.5px}}" +
      ".b3{display:inline-flex;gap:5px;align-items:flex-end;flex:0 0 auto}" +
      ".b3 .b3i{display:inline-flex;flex-direction:column;align-items:center;gap:1px}" +
      ".b3 .b3t{display:flex;align-items:flex-end;width:8px;height:18px;background:rgba(28,36,51,.09);border-radius:2px;overflow:hidden}" +
      ".b3 .b3t b{display:block;width:100%;border-radius:1px 1px 0 0}" +
      ".b3 .b3l{font-size:7.5px;font-weight:800;color:#8a93a3;line-height:1}" +
