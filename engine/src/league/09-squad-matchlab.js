  ...
  function foSqxCss() {
    ...
    s.textContent = [
      ...
      ".fo-sqx-insp{margin:10px auto 0;max-width:520px;background:linear-gradient(180deg,rgba(11,25,48,.86),rgba(5,13,24,.9));border:1px solid rgba(235,194,113,.5);border-radius:16px;padding:15px 16px;backdrop-filter:blur(12px);box-shadow:0 16px 40px rgba(0,0,0,.4)}",
      "@media(min-width:1024px){.fo-sqx-insp{margin:0;max-width:none;position:sticky;top:8px}}",
      ".fo-sqx-ihd{display:flex;align-items:flex-start;justify-content:space-between;gap:10px}",
      ".fo-sqx-irole{font-family:Oswald,sans-serif;text-transform:uppercase;letter-spacing:2px;font-size:10px;color:#EBC271}",
      ".fo-sqx-iname{font-family:Oswald,sans-serif;font-weight:600;font-size:22px;color:#F5EFDC;line-height:1.05;margin-top:2px}",
      ".fo-sqx-iovr{text-align:center;flex:none}.fo-sqx-iovr small{display:block;font-family:Oswald,sans-serif;font-size:9px;letter-spacing:2px;color:#9AA9BD}.fo-sqx-iovr b{font-family:Oswald,sans-serif;font-weight:700;font-size:26px;color:#fff;line-height:1}",
      ".fo-sqx-istat{display:flex;align-items:center;justify-content:space-between;gap:10px;margin:12px 0 10px;font-family:Oswald,sans-serif;text-transform:uppercase;letter-spacing:1px;font-size:11px}",
      ".fo-sqx-istat .xi{color:#24D3D8}.fo-sqx-istat .bench{color:#9AA9BD}.fo-sqx-istat .ok{color:#4DA6A2}.fo-sqx-istat .lo{color:#E6532F}",
      ".fo-sqx-ifl{font-family:Oswald,sans-serif;text-transform:uppercase;letter-spacing:1.4px;font-size:10px;color:#9AA9BD;display:flex;align-items:baseline;gap:8px}.fo-sqx-ifl b{color:#F5EFDC;font-size:14px}.fo-sqx-ifl i{font-style:normal;color:#EBC271;margin-left:auto}",
      ".fo-sqx-imeter{height:7px;border-radius:99px;background:rgba(255,255,255,.09);overflow:hidden;margin-top:6px}",
      ".fo-sqx-imeter i{display:block;height:100%;background:linear-gradient(90deg,#E4AD35,#EBC271)}",
  ...
  window.pgSquad = function () {
    try {
      ...
      var tabs = [["xi", "XI"], ["all", "ALL"], ["bat", "BAT"], ["bowl", "BOWL"], ["keep", "KEEP"], ["youth", "YOUTH"]].map(function (tb) {
        return "<button class='fo-sqx-tab" + (sv.tab === tb[0] ? " on" : "") + "' data-tab='" + tb[0] + "'>" + tb[1] + "</button>";
      }).join("");
      // selected-player inspector / action drawer, from live values
      var inspHTML = "";
      if (sel) {
        var iEn = foEnergyOf(sel), iRole = foPkRoleLbl(sel), iOvr = foPkOvr(sel), inXi = !!xiSet[sel.name];
        var iTal = (sel.talents || []).slice(0, 2).map(function (t2) { return "<span class='fo-sqx-itag'>" + E(ptal(t2)) + "</span>"; }).join("");
        inspHTML = "<aside class='fo-sqx-insp'>" +
          "<div class='fo-sqx-ihd'><div class='fo-sqx-iid'><div class='fo-sqx-irole'>" + E(iRole) + "</div><div class='fo-sqx-iname'>" + E(sel.name) + "</div></div><div class='fo-sqx-iovr'><small>OVR</small><b>" + iOvr + "</b></div></div>" +
          "<div class='fo-sqx-istat'><span class='" + (inXi ? "xi" : "bench") + "'>" + (inXi ? "&#10003; In tonight's XI" : "On the bench") + "</span><span class='" + (iEn.tired ? "lo" : "ok") + "'>" + (iEn.tired ? "Low fitness" : "Match ready") + "</span></div>" +
          "<div class='fo-sqx-ifit'><div class='fo-sqx-ifl'>Fitness <b>" + iEn.pct + "%</b><i>" + E(iEn.word) + "</i></div><div class='fo-sqx-imeter'><i style='width:" + iEn.pct + "%'></i></div></div>" +
          (iTal ? "<div class='fo-sqx-italents'>" + iTal + "</div>" : "") +
          "<div class='fo-sqx-iact'><button type='button' class='fo-sqx-ib view'>View player</button><button type='button' class='fo-sqx-ib edit'>Edit XI &#8594;</button></div>" +
          "</aside>";
      }
      ...
      page.innerHTML =
        ...
        "</header>" +
        "<div class='fo-sqx-stage'>" +
        "<div class='fo-sqx-deckwrap'>" +
      ...
      // inspector actions: reuse the real player + lineup routes
      var iv = page.querySelector(".fo-sqx-ib.view"); if (iv) iv.addEventListener("click", function () { if (sv.sel) location.hash = "#/player?n=" + encodeURIComponent(sv.sel); });
      var ie = page.querySelector(".fo-sqx-ib.edit"); if (ie) ie.addEventListener("click", function () { location.hash = "#/orders"; });
