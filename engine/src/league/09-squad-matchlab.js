  ...
  function foSqxCss() {
    ...
    s.textContent = [
      ...
      // body: gallery + rail
      ".fo-sqx-body{display:block;flex:1;min-height:0}",
      // a big-card RAIL of the collectible phc cards you sweep left to right
      ".fo-sqx-gallery{display:flex;gap:16px;overflow-x:auto;overflow-y:hidden;padding:10px 6px 16px;align-items:flex-start;scroll-snap-type:x proximity;scroll-behavior:smooth}",
      ...
      // EVERY card is one fixed size for symmetry: the phc is rendered at a
      // comfortable native width and zoomed as a whole (so its internal layout
      // never breaks), with a fixed native height whose ART flexes to absorb any
      // difference in talent/meta lines — so all cards end up identical.
      ".fo-sqx-card{position:relative;flex:0 0 auto;cursor:pointer;scroll-snap-align:start;zoom:.6;transition:transform .16s ease;will-change:transform}",
      ".fo-sqx-card:hover{transform:translateY(-8px)}",
      ...
      "@media(max-width:900px){.fo-sqx-hd{gap:12px}.fo-sqx-sort{margin-left:0}.fo-sqx-card{zoom:.72}}"
    ].join("");
  ...
  window.pgSquad = function () {
    try {
      ...
      var cards = pool.map(function (p) {
        var hc = foHoloCardHTML(p, t.name);   // the collectible phc card + its rarity tier
        return "<div class='fo-sqx-card ph-" + hc.tier + (xiSet[p.name] ? " inxi" : "") + "' data-n='" + E(p.name) + "'>" + hc.html + "</div>";
      ...
      page.querySelectorAll(".fo-sqx-rar").forEach(function (b) { b.addEventListener("click", function () { sv.rarity = b.getAttribute("data-r"); pgSquad(); }); });
      // a tap on a card opens that player's full page
      page.querySelectorAll(".fo-sqx-card[data-n]").forEach(function (c) { c.addEventListener("click", function () { location.hash = "#/player?n=" + encodeURIComponent(c.getAttribute("data-n")); }); });
