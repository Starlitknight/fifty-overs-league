...
(function(){
  ...
  if(!window.__fo11_background_tick){window.__fo11_background_tick=setInterval(function(){try{if(typeof M==='undefined'||!M||M.done||!M.innings)return;if(window.__ap)return;doBall();if((location.hash||'').split('?')[0]==='#/match'&&typeof renderMatch==='function')renderMatch();}catch(e){console.warn('[v11 background sim]',e);}},1400);}
