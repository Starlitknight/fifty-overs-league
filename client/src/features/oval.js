...
FOC.oval = (function () {
  ...
  function stageHTML() {
    return "<div id='fo-oval' title='Ball-by-ball theatre — outcomes are real; shot directions are illustrative.'>" +
      ...
      "</svg><div class='ov-note'>theatre · directions illustrative</div></div>";
  }
  ...
  function shatter() {
    var st = document.getElementById("ov-stumps-a");
    if (!st) return;
    st.style.transition = "transform .3s ease";
    st.style.transformOrigin = "200px 182px";
  ...
  function tick() {
    try {
      if (location.hash.indexOf("#/match") !== 0) { seenLogLen = 0; return; }
      if (typeof M === "undefined" || !M || !M.log) return;
      var page = document.getElementById("page");
      if (!page) return;
      if (!document.getElementById("fo-oval")) {
        var host = page.querySelector(".matchwrap, .card, div");
  ...
  function css() {
    ...
    st.textContent =
      "#fo-oval{max-width:560px;margin:0 auto 14px;background:#0F1A2E;border:1px solid #24334f;border-radius:14px;overflow:hidden}" +
