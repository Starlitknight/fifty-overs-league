...
(function () {
  ...
  css3.textContent =
    ...
    "#topbar a[data-nav=\"reports\"],#topbar a[data-nav=\"manual\"]{display:none !important}" +
  ...
  function ensureNav() {
    try {
      ...
      if (tb.querySelector("a.fo-logout")) return;
      ...
      var f = mk("Practice Game", "fo-friendly", startFriendly);
      var lg = mk("League", "fo-league", openLeagueMenu);   // menu: admin controls
      var out = mk("Log out", "fo-logout", doLogout);
      if (after && after.nextSibling) { tb.insertBefore(f, after.nextSibling); tb.insertBefore(lg, f.nextSibling); tb.insertBefore(out, lg.nextSibling); }
      else { tb.appendChild(f); tb.appendChild(lg); tb.appendChild(out); }
    } catch (e) {}
  }
    ...
    try {
      ...
      document.querySelectorAll("#page table").forEach(function (tb) {
        ...
        tb.querySelectorAll("tr").forEach(function (tr) {
          ...
          var cell = tr.children[clubIx]; if (!cell || cell.dataset.foScout) return;
  ...
  try {
    ...
    if (pg0 && window.MutationObserver) new MutationObserver(function () { clearTimeout(_mt); _mt = setTimeout(function () { decorateFixtureTimes(); tidyPage(); }, 40); }).observe(pg0, { childList: true, subtree: true });
  } catch (e) {}
  if (typeof window.route === "function") { var _rt = window.route; window.route = function () { var r = _rt.apply(this, arguments); bumpBrand(); ensureNav(); decorateFixtureTimes(); tidyPage(); return r; }; }
