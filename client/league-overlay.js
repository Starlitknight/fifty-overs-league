...
(function () {
  ...
  function foFrDoneRender(c, id) {
    ...
    // player of the match = top fantasy points, the same banner league
    // scorecards carry
    var pom = "";
    try {
      var topF = r.fantasy && r.fantasy[0];
      if (topF) {
        pom = "<div class='fo-pom'><div class='fo-pom-k'>Player of the match</div><div class='fo-pom-n'>" + E(topF.n) + "</div>" +
          "<div class='fo-pom-s'>" + E(topF.team || "") + " &middot; <b>" + (topF.pts || 0) + " fantasy pts</b> (bat " + (topF.bat || 0) + " &middot; bowl " + (topF.bowl || 0) + " &middot; field " + (topF.field || 0) + ")</div></div>";
      } else if (r.mom) {
        pom = "<div class='fo-pom'><div class='fo-pom-k'>Player of the match</div><div class='fo-pom-n'>" + E(r.mom) + "</div></div>";
      }
    } catch (ePom) {}
