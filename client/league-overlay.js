...
(function () {
  ...
  function foManualPage() {
    ...
    var secs = [
      ...
      ["pitchwx", "Pitch, weather &amp; the toss", [
        ...
        "<p><b>Heat is a selection question.</b> In humid, hot and scorching weather every player&rsquo;s fatigue clock runs faster. Tired bowlers lose threat and control late in spells, set batters lose their edge sooner, and heavy workloads leave players tired for the next matchday at lower ball counts: on a scorching day a bowler is feeling the next morning after roughly six overs instead of eight. Six bowling options beat five on draining days; shorter spells keep everyone dangerous, and the order screen warns you whenever the forecast drains.</p>",
  ...
  // ---- weather that drains: hot conditions tire players faster (real, in-engine) ----
  ...
  // each ball's batter, bowler and weather, so it is the one honest seam: in
  // draining weather both players' tire rate is raised before the engine rolls
  // the ball; in kind weather the override is cleared. Because raw fatigue
  // itself accrues faster, the engine's own tired warnings, fatigue bars and
  // performance penalties all stay in agreement with the mechanic.
  ...
  try {
    if (typeof window.ballDist === "function" && !window.ballDist.__foWx) {
      ...
      window.ballDist = function (bat, bowl, ph, faced, intent, rrDef, pitch, field, over, ctx) {
        try {
          var f = ctx && FO_WX_DRAIN[String(ctx.weather || "").toLowerCase()];
          if (f) {
            var base = (typeof foAgeTireFactor === "function") ? foAgeTireFactor : function () { return 1; };
            bat._ageTire = base(bat) * f; bowl._ageTire = base(bowl) * f;
          } else {
            if (bat._ageTire) delete bat._ageTire;
            if (bowl._ageTire) delete bowl._ageTire;
          }
        } catch (e) {}
    ...
    // the engine's post-match tired thresholds shrink by sqrt(drain), a gentler
    // scale than the in-match effect so it singles out the flogged workhorses
    // (a 9-over spell in the heat) instead of blanket-tiring both squads.
    // Runs after the engine's own pass and only ever ADDS tiredness.
    if (typeof window.saveMatch === "function" && !window.saveMatch.__foWx) {
      ...
      window.saveMatch = function (m) {
        ...
        try {
          var f = m && m.meta && FO_WX_DRAIN[String(m.meta.weather || "").toLowerCase()];
          if (f && typeof findPlayer === "function") {
            var g = Math.sqrt(f);
            ...
            for (var ii = 0; ii < (m.innings || []).length; ii++) {
              ...
              (inn.bat || []).forEach(function (b) {
                ...
                if (b.b >= ((pl.p.keeper || b.p.name === capt) ? 75 : 90) / g) pl.p.fatigue = "tired";
              });
              for (var k in (inn.bowlers || {})) {
                var br = inn.bowlers[k]; if (!br) continue;
                var pl2 = findPlayer(k); if (!pl2 || !pl2.p) continue;
                if (br.b >= ((pl2.p.keeper || k === capt) ? 42 : 48) / g) pl2.p.fatigue = "tired";
              }
            }
          }
        } catch (e) {}
