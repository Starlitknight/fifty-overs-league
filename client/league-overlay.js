...
(function () {
  ...
  function foOrdColors() {
    var names = [], g = App.orders.grid || [];
    for (var o = 1; o <= 50; o++) if (g[o] && names.indexOf(g[o]) < 0) names.push(g[o]);
    var map = {}; names.forEach(function (nm, i) { map[nm] = FO_ORD_COLS[i % FO_ORD_COLS.length]; });
    return map;
  }
  ...
  function foOrdBowlBody() {
    ...
    var blocks = FO_ORD_PH.map(function (ph, ix) {
      var slots = [1, 0].map(function (par) {
        ...
        var inner = own
          ? "<b style='color:" + (colors[own.nm] || "#0E233F") + "'>" + E(foOrdSurname(own.nm)) + "</b><span>" + own.n + " ov" + (own.mixed || own.n < own.tot ? " · partial" : "") + "</span>"
          : "<b class='fo-os-ai'>AI decides</b><span>tap to pick</span>";
        return "<button class='fo-os-slot" + (own ? " set" : "") + "' data-fo-slot='" + ix + ":" + par + "'><i>" + lblEnd + "</i>" + inner + "</button>";
      }).join("");
  ...
  function foOrdersUI() {
    ...
    var tac = "<div class='fo-ord-tac'>" +
      ...
      "<div class='fo-ord-tr'><span>Powerplay</span>" + sel2("pi:pp", INT, App.orders.phaseIntent.pp) + "<span>Middle</span>" + sel2("pi:mid", INT, App.orders.phaseIntent.mid) + "<span>Death</span>" + sel2("pi:death", INT, App.orders.phaseIntent.death) + "</div>" +
      "<div class='fo-ord-tach'>Field when bowling</div>" +
      "<div class='fo-ord-tr'><span>Powerplay</span>" + sel2("fp:pp", FLD, App.orders.fieldPlan.pp) + "<span>Middle</span>" + sel2("fp:mid", FLD, App.orders.fieldPlan.mid) + "<span>Death</span>" + sel2("fp:death", FLD, App.orders.fieldPlan.death) + "</div>" +
      "<div class='fo-ord-tach'>Toss</div>" +
      "<div class='fo-ord-tr'><span>Call</span>" + sel2("toss:call", [["", "Ask me"], ["H", "Heads"], ["T", "Tails"]], App.orders.tossCall || "") + "<span>If won</span>" + sel2("toss:dec", [["bat", "Bat"], ["bowl", "Bowl"]], App.orders.tossDecision || "bat") + "</div></div>";
  ...
  try {
    ...
    foOrdCss.textContent =
      ...
      "html body #page button.fo-ob-chip{flex:0 0 auto;border:1px solid rgba(28,36,51,.2);background:#FFFEFC;color:#5a6472;border-radius:7px;padding:3px 7px;font-size:10.5px;font-weight:800;cursor:pointer}" +
      "html body #page button.fo-ob-chip.on{background:#0E233F;color:#FFFEFC;border-color:#0E233F}" +
      ".fo-ob-mv{display:flex;flex-direction:column;gap:1px}" +
      "html body #page .fo-ob-mv button{border:1px solid rgba(28,36,51,.14);background:#FBFAF7;color:#5a6472;border-radius:5px;font-size:8px;line-height:1;padding:3px 6px;cursor:pointer}" +
      "html body #page .fo-ob-mv button:disabled{opacity:.3;cursor:default}" +
      ...
      "html body #page button.fo-os-slot{display:block;text-align:left;border:1px dashed rgba(28,36,51,.25);background:#FFFEFC;border-radius:9px;padding:6px 9px;cursor:pointer;min-width:0}" +
      "html body #page button.fo-os-slot.set{border-style:solid}" +
      ".fo-os-slot i{display:block;font-style:normal;font-size:9px;letter-spacing:.06em;text-transform:uppercase;color:#a7aeba;font-weight:800}" +
      ".fo-os-slot b{display:block;font-size:12.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}" +
      ...
      ".fo-ord-tach{font-size:10px;letter-spacing:.07em;text-transform:uppercase;color:#8a93a3;font-weight:800;margin:7px 0 3px}" +
      ".fo-ord-tr{display:flex;align-items:center;gap:6px;flex-wrap:wrap;font-size:11.5px;color:#5a6472}" +
      ".fo-ord-tr select{font-size:12px;padding:3px 5px;border-radius:7px}" +
      ...
      try {
        ...
        if (rmTb) {
          ...
          frAll2.forEach(function (c) {
            ...
            items.push({ at: at2, fr: true, id: c.id, teams: teams2, bat: bat2, bowl: bowl2, dateTxt: E(d2.toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })) + " " + pad0(d2.getHours()) + ":" + pad0(d2.getMinutes()) });
          });
