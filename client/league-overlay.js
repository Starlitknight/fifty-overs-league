...
(function () {
  ...
  function foRenderMatchday() {
    try {
      if (/^#\/matchday/.test(location.hash || "") && typeof foLiveBoardHTML === "function") {
        var lb = foLiveBoardHTML();
        var pgM = document.getElementById("page");
        if (lb && pgM && !pgM.querySelector(".fo-live-row")) {
          setTimeout(function () { if (!pgM.querySelector(".fo-live-row")) pgM.insertAdjacentHTML("afterbegin", lb); }, 80);
        }
      }
    } catch (eL) {}
  ...
  try { (function () {
    ...
    function foBcastT0(rd) {
      ...
      try {
        var ds = foDailyDate(rd); var t = Date.parse(ds + " 09:00:00 GMT-0400");
        if (!isNaN(t)) return t;
      } catch (e2) {}
      return 0;
    }
