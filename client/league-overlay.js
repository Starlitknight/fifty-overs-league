...
(function () {
  ...
  try { (function () {
    ...
    function foChartFrame(maxY, yStep, maxX, yl) {
      var W = 560, H = 232, L = 36, B = 24, T = 10, R = 10;
      var gx = function (x) { return L + (W - L - R) * x / Math.max(1, maxX); };
      var gy = function (y) { return H - B - (H - B - T) * y / Math.max(1, maxY); };
      var grid = "";
      for (var y = 0; y <= maxY; y += yStep) grid += "<line x1='" + L + "' y1='" + gy(y) + "' x2='" + (W - R) + "' y2='" + gy(y) + "' stroke='#e3ded2' stroke-dasharray='4 4'/><text x='" + (L - 6) + "' y='" + (gy(y) + 3.5) + "' text-anchor='end' font-size='10' fill='#8a8474'>" + y + "</text>";
      for (var x = 0; x <= maxX; x += 10) grid += "<text x='" + gx(x) + "' y='" + (H - 8) + "' text-anchor='middle' font-size='10' fill='#8a8474'>" + x + "</text>";
      return { W: W, H: H, gx: gx, gy: gy, open: "<svg viewBox='0 0 " + W + " " + H + "' style='width:100%;height:auto'>" + grid, yl: yl };
    }
    ...
    window.foMatchCharts = function (all, worms) {
      ...
      var mY = Math.ceil((maxR + 2) / 5) * 5;
      var f1 = foChartFrame(mY, 5, maxO + 1, "runs / over");
      var bw = Math.max(2.4, (f1.W - 46) / (maxO + 1) / 2.6);
      var man = f1.open;
      series.forEach(function (sr, si) {
        sr.forEach(function (pt, i) {
          var rv = pt.runs - (i ? sr[i - 1].runs : 0), wv = pt.wkts - (i ? sr[i - 1].wkts : 0);
          var x = f1.gx(pt.o) + (si ? 1 : -1 - bw);
          man += "<rect x='" + x + "' y='" + f1.gy(rv) + "' width='" + bw + "' height='" + (f1.gy(0) - f1.gy(rv)) + "' rx='1' fill='" + (si ? C2 : C1) + "'/>";
          if (wv > 0) man += "<circle cx='" + (x + bw / 2) + "' cy='" + (f1.gy(rv) - 5) + "' r='2.6' fill='none' stroke='#a33328' stroke-width='1.6'/>";
        });
      });
      man += "</svg>";
      // Run rate + Worm lines
      var line = function (fr, sr, val, col) {
        var d = "M" + fr.gx(0) + " " + fr.gy(0);
        sr.forEach(function (pt) { d += " L" + fr.gx(pt.o) + " " + fr.gy(val(pt)); });
        return "<path d='" + d + "' fill='none' stroke='" + col + "' stroke-width='2.2' stroke-linejoin='round'/>";
      };
      ...
      var rr = f2.open;
      series.forEach(function (sr, si) { rr += line(f2, sr, function (pt) { return pt.runs / pt.o; }, si ? C2 : C1); });
      rr += "</svg>";
      var maxRuns = Math.max.apply(null, series.map(function (sr) { return sr.length ? sr[sr.length - 1].runs : 0; }).concat([10]));
      var f3 = foChartFrame(Math.ceil((maxRuns + 15) / 20) * 20, 40, maxO + 1, "runs");
      var wm = f3.open;
      series.forEach(function (sr, si) {
        wm += line(f3, sr, function (pt) { return pt.runs; }, si ? C2 : C1);
        sr.forEach(function (pt, i) { if (pt.wkts > (i ? sr[i - 1].wkts : 0)) wm += "<circle cx='" + f3.gx(pt.o) + "' cy='" + f3.gy(pt.runs) + "' r='2.8' fill='#a33328'/>"; });
      });
      wm += "</svg>";
      // Partnerships
      var pships = all.map(function (inn) {
        ...
        return "<div class='fo-an-psh'><div class='fo-an-psht'>" + E(inn.batTeam) + "</div>" + ps.map(function (q) {
          var pr = (q.pair || " / ").split(" / ");
          return "<div class='fo-psh-row'><span class='fo-psh-l'>" + E(pr[0] || "") + "</span>" +
            "<span class='fo-psh-m'><b>" + q.runs + " (" + q.balls + ")</b><i style='width:" + Math.max(3, Math.round(100 * q.runs / mx)) + "%'></i></span>" +
            "<span class='fo-psh-r'>" + E(pr[1] || "") + "</span></div>";
        }).join("") + "</div>";
      }).join("");
      var sec = function (t, b) { return "<div class='panel'><h4>" + t + "</h4><div class='pad'>" + b + "</div></div>"; };
      return sec("Partnerships", pships) +
        sec("Manhattan", foLegend(all) + man) +
        sec("Run rate", foLegend(all) + rr) +
        sec("Worm", foLegend(all) + wm);
    };
    // ---- the scorecard ----
    ...
    cs.textContent =
      ...
      ".fo-psh-m i{display:block;height:7px;border-radius:99px;background:linear-gradient(90deg,#2d6a8f,#4DA6A2);margin:3px auto 0}" +
