...
(function () {
  ...
  if (typeof window.route === "function") { var _rt = window.route; window.route = function () { var r = _rt.apply(this, arguments); bumpBrand(); ensureNav(); foRenderTraining(); foPolishSquad(); foDecorateMatchRows(); foRenderScout(); decorateFixtureTimes(); tidyPage(); foTagMatchPage(); foRenderPlanner(); foOrdersExtras(); foHidePlayerSkills(); return r; }; }
  ...
  var FO_PITCHES = [
    ...
    { id: "dry", nm: "Dry", d: "Turns square. Spinners thrive; draft slow bowlers." },
    ...
    { id: "cracked", nm: "Cracked", d: "Unpredictable — wickets for everyone. Chaos cricket." }
  ];
  ...
  function foOnbCreate() {
    ...
    var body =
      ...
      "<label class='fo-ob-lbl'>Home pitch <span class='fo-ob-hint'>&mdash; you play 9 of your 18 matches on it, so draft a squad to match</span></label>" +
      "<div class='fo-pitchgrid'>" + FO_PITCHES.map(function (pt) {
  ...
  function foOnbCharter() {
    FO_ONB.step = 2;
    var pt = FO_PITCHES.find(function (x) { return x.id === FO_ONB.pitch; }) || FO_PITCHES[0];
    var body =
      "<div class='fo-ob-card fo-ob-narrow fo-ob-charter'>" +
      "<div class='fo-charter-ic'>" + FO_I("trophy", 30) + "</div>" +
      "<div class='fo-ob-eyebrow'>The charter is signed</div>" +
      "<h1 class='fo-ob-h1'>" + E(FO_ONB.clubName) + " is founded</h1>" +
      "<p class='fo-ob-lead'>The board is seated, the groundsman has the keys to <b>" + E(FO_ONB.ground) + "</b>" +
      (pt.id !== "balanced" ? " — a <b>" + pt.nm.toLowerCase() + "</b> deck" : "") +
      ", and your founding sponsor has wired <b>$1,000,000</b> to get the club off the ground.</p>" +
      "<div class='fo-charter-grant'><span>Founding grant</span><b>$1,000,000</b><span>draft budget + operating money</span></div>" +
      "<p class='fo-ob-lead'>Here's the road to your first season:</p>" +
      "<div class='fo-ob-chks' style='text-align:left'>" +
      "<div class='fo-ob-chk'><i>" + FO_I("coins", 17) + "</i><span>Learn how the money works — it decides everything.</span></div>" +
      "<div class='fo-ob-chk'><i>" + FO_I("users", 17) + "</i><span>Choose a matchday sponsor — your main income all season.</span></div>" +
      "<div class='fo-ob-chk'><i>" + FO_I("bat", 17) + "</i><span>Draft 14&#8211;16 players suited to your " + pt.nm.toLowerCase() + " pitch.</span></div>" +
      "<div class='fo-ob-chk'><i>" + FO_I("calendar", 17) + "</i><span>Play 18 matchdays — one match resolves every day at 9:00 AM ET.</span></div>" +
      "</div>" +
  ...
  function foOnbSponsor() {
    ...
    var body =
      ...
      "<h1 class='fo-ob-h1'>Choose your sponsor &mdash; they pay you every matchday</h1>" +
      "<p class='fo-ob-lead'>This is your club's main income for the whole season, paid after every match. Safe money, results money, or back yourself for the big bonuses — the choice locks in for Season 1.</p>" +
      ...
      "<div class='fo-ob-act fo-ob-act-c'><button class='fo-ob-ghost' id='fo-ob-b'>Back</button><button class='fo-ob-cta' id='fo-ob-c'" + (FO_ONB.sponsor ? "" : " disabled") + ">Continue to draft</button></div></div>";
  ...
  function foAdvisor(picked, fc, styleId) {
    ...
    if ((pt === "green" || pt === "cracked") && s.n >= 6 && picked.filter(function (p) { return p.bowlTypeFull && p.bowlTypeFull !== "none" && typeof isPace === "function" && isPace(p); }).length < 3) out.push({ t: "info", m: "Your home pitch is " + pt + " — pace bowlers will love it. Consider drafting more seamers." });
    if (pt === "dry" && s.n >= 6 && picked.filter(function (p) { return p.bowlTypeFull && p.bowlTypeFull !== "none" && !(typeof isPace === "function" && isPace(p)); }).length < 2) out.push({ t: "info", m: "Your home pitch is dry — it will turn. Consider drafting more spinners." });
  ...
  function foYouthDetail(p) {
    ...
    d.innerHTML = "<div class='fo-pd-back'><div class='fo-pd-card'>" +
      ...
      "<div class='fo-pd-sec'>Skill summary</div><div class='fo-pd-bars'>" + barHtml + "</div>" +
      "<div class='fo-pd-tal'><b>Talents:</b> " + talents + "</div>" +
      "<div class='fo-pd-act'><button class='fo-pd-add'>Sign " + E(p.name.split(" ")[0]) + " — " + FO$(p.fee) + "</button></div>" +
      "</div></div>";
    document.body.appendChild(d);
    d.querySelector(".fo-pd-x").addEventListener("click", function () { d.remove(); });
    d.querySelector(".fo-pd-back").addEventListener("click", function (e) { if (e.target.classList.contains("fo-pd-back")) d.remove(); });
    d.querySelector(".fo-pd-add").addEventListener("click", function () { d.remove(); foSignYouth(p); });
  }
  ...
  function foTrainingPage() {
    ...
    page.innerHTML =
      ...
      "<table class='fo-tr-tbl'><thead><tr><th>Player</th><th>Potential</th><th>Fatigue</th><th>Program</th><th>Next gain</th></tr></thead><tbody>" + rows + "</tbody></table>" +
  ...
  function foDecorateMatchRows() {
    try {
      ...
      page.querySelectorAll('tr[style*="eef4ee"], tr[style*="eef8fb"]').forEach(function (tr) {
        ...
        b.className = "fo-setr"; b.textContent = done ? "Edit lineup" : "Set lineup"; b.style.marginLeft = "8px";
