...
(function () {
  ...
  function foPremiumClub() {
    try {
      ...
      var msCard = "<div class='fo-card'><div class='fo-card-h2row'><div class='fo-card-h2'>Milestone watch</div><a href='#/museum' class='fo-morelink'>Club museum &rsaquo;</a></div><div class='fo-card-b'>" +
        (msRows.length ? msRows.map(function (r2) { return "<div class='fo-ms-row'><b>" + E(r2.nm) + "</b><span>" + r2.txt + "</span></div>"; }).join("")
  ...
  function foClubTotals(name) {
    var h = (App.playerHist && App.playerHist[name]) || [], runs = 0, wkts = 0, hs = 0, hsTxt = null, bw = 0, bwr = 1e9, bbTxt = null;
  ...
  function foChroniclePre() {
    ...
    try {
      GD.teams.forEach(function (t) {
        (t.players || []).forEach(function (p) {
          var h = (App.playerHist && App.playerHist[p.name]) || [], runs = 0, w = 0;
          h.forEach(function (e) { runs += +e.rr || 0; w += +e.w || 0; });
          pre.players[p.name] = { n: h.length, runs: runs, wkts: w };
        });
      });
    } catch (e) {}
    return pre;
  ...
  function foChroniclePost(pre) {
    try {
      ...
      GD.teams.forEach(function (t) {
        ...
        (t.players || []).forEach(function (p) {
          var h = (App.playerHist && App.playerHist[p.name]) || [];
          var was = pre.players[p.name] || { n: 0, runs: 0, wkts: 0 };
          var newE = h.slice(was.n);
          if (!newE.length) return;
          newE.forEach(function (e) { e.s = sN; e.r = rN; });
          if (!was.n) { foCareerPush(p, { s: sN, r: rN, ev: "debut", txt: "Debut for " + t.name }); feed.push({ ev: "debut", name: p.name }); }
      ...
      // 3. season awards on the final round, into the museum + the career
      ...
      if (rN >= total) {
        GD.teams.forEach(function (t) {
          ...
          (t.players || []).forEach(function (p) {
            ...
            h.forEach(function (e) { if (e.s === sN) { rr += +e.rr || 0; w += +e.w || 0; } });
            var v = rr + 20 * w;
            if (v > bestV) { bestV = v; best = { p: p, rr: rr, w: w }; }
            if ((p.age || 30) <= 23 && v > yBestV) { yBestV = v; yBest = { p: p, rr: rr, w: w }; }
          });
          var mus = (t._museum = t._museum || { trophies: [], awards: [], legends: [] });
          if (best) {
            mus.awards.push({ s: sN, kind: "Player of the Season", name: best.p.name, line: best.rr + " runs · " + best.w + " wkts" });
            foCareerPush(best.p, { s: sN, r: rN, ev: "award", txt: "Player of the Season, S" + sN });
          }
          if (yBest && (!best || yBest.p !== best.p)) {
            mus.awards.push({ s: sN, kind: "Young Player of the Season", name: yBest.p.name, line: yBest.rr + " runs · " + yBest.w + " wkts" });
            foCareerPush(yBest.p, { s: sN, r: rN, ev: "award", txt: "Young Player of the Season, S" + sN });
          }
        });
      }
    } catch (e) {}
  ...
  try {
    if (typeof window.completeRound === "function" && !window.completeRound.__foChron) {
      ...
      window.completeRound = function () {
        var pre = null;
        try { pre = foChroniclePre(); } catch (e) {}
  ...
  function foVoiceItem(t, rdHuman) {
    try {
      ...
      (t.players || []).forEach(function (p) {
        var h = (App.playerHist && App.playerHist[p.name]) || [];
        var recent = h.filter(function (e) { return e.s === sN; }).slice(-2);
  ...
  // ---- milestone watch: who is N short of a number that matters ----
  function foMilestoneWatch(t) {
    var rows = [];
    (t.players || []).forEach(function (p) {
      var tot = foClubTotals(p.name);
      if (!tot.matches) return;
      var nextR = (Math.floor(tot.runs / 500) + 1) * 500, gapR = nextR - tot.runs;
      if (gapR <= 40) rows.push({ nm: p.name, txt: "needs <b>" + gapR + "</b> runs for " + nextR.toLocaleString() + " club runs", w: gapR / 40 });
      var nextW = (Math.floor(tot.wkts / 25) + 1) * 25, gapW = nextW - tot.wkts;
      if (tot.wkts > 0 && gapW <= 4) rows.push({ nm: p.name, txt: "needs <b>" + gapW + "</b> wicket" + (gapW === 1 ? "" : "s") + " for " + nextW + " for the club", w: gapW / 4 });
      if (tot.hs >= 75 && tot.hs < 100) rows.push({ nm: p.name, txt: "best of <b>" + tot.hs + "</b> · a maiden century is close", w: (100 - tot.hs) / 25 });
      else if (tot.hs >= 35 && tot.hs < 50) rows.push({ nm: p.name, txt: "best of <b>" + tot.hs + "</b> · chasing a maiden fifty", w: (50 - tot.hs) / 15 });
    });
  ...
  function foCareerPanel() {
    try {
      ...
      var h = (App.playerHist && App.playerHist[name]) || [];
      var bySeason = {};
      h.forEach(function (e) {
        var k = e.s || "early";
  ...
  function foMuseumHTML(t) {
    ...
    var awards = (mus.awards || []).slice().reverse().map(function (a) {
      return "<tr><td>S" + a.s + "</td><td>" + E(a.kind) + "</td><td><b>" + E(a.name) + "</b></td><td class='small'>" + E(a.line || "") + "</td></tr>";
    }).join("") || "<tr><td colspan='4' class='small'>Season awards are handed out on the final matchday.</td></tr>";
