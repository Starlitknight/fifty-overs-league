...
(function () {
  ...
  css3.textContent =
    ...
    "#topbar{overflow-x:auto;-webkit-overflow-scrolling:touch;flex-wrap:nowrap}" +
    "#topbar a{padding:11px 12px;font-size:13px}#topbar .brand{font-size:14px}" +
    ...
    "#page table{font-size:12px;width:100%}" +
    "button,select,input{min-height:38px;font-size:14px}" +
  ...
  if (typeof window.route === "function") { var _rt = window.route; window.route = function () { var r = _rt.apply(this, arguments); bumpBrand(); return r; }; }
  ...
  function practice() {
    sel("league_clubs", "league_id=eq." + LG.id + "&manager_id=eq." + SYNC.myMid + "&select=club").then(function (rows) {
      ...
      world = world.concat(defaults.filter(function (t) { return !used[t.name]; }).slice(0, 3));
      GD.teams = world;
      if (typeof window.econInit === "function") window.econInit();
      App.teamIx = 0; App.season = null; if (typeof window.seasonInit === "function") window.seasonInit();
