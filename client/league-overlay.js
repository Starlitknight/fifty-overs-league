...
(function () {
  ...
  function foRenderPredictions() {
    ...
    // the server is the truth for MY picks too: restore highlights from it, so
    // they survive reloads, other devices, and anything localStorage forgot
    sel("league_predictions", "league_id=eq." + LG.id + "&manager_id=eq." + SYNC.myMid + "&round=eq." + rd + "&select=picks").then(function (rows2) {
      ...
      var merged = false;
      for (var k2 in srv) { if (mine[k2] !== srv[k2]) { mine[k2] = srv[k2]; merged = true; } }
      if (merged) lsSet(foPredKey(rd), JSON.stringify(mine));
      host.querySelectorAll(".fo-pick").forEach(function (b2) {
        var on = mine[b2.getAttribute("data-k")] === b2.getAttribute("data-v");
        b2.classList.toggle("fo-pick-on", on);
      });
    }).catch(function () {});
