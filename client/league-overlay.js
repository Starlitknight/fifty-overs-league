...
(function () {
  ...
  function foChroniclePre() {
    ...
    try {
      GD.teams.forEach(function (t) {
        (t.players || []).forEach(function (p) {
          var h = (App.playerHist && App.playerHist[p.name]) || [], runs = 0, w = 0, hs = 0, bw = 0, bwr = 1e9;
          h.forEach(function (e) {
            runs += +e.rr || 0; w += +e.w || 0;
            if ((+e.rr || 0) > hs) hs = +e.rr;
            if ((+e.w || 0) > bw || ((+e.w || 0) === bw && (+e.w || 0) > 0 && (+e.cr || 0) < bwr)) { if (+e.w) { bw = +e.w; bwr = +e.cr || 0; } }
          });
          pre.players[p.name] = { n: h.length, runs: runs, wkts: w, hs: hs, bw: bw, bwr: bwr };
        });
      });
    } catch (e) {}
    return pre;
  ...
  function foMilestoneWatch(t, max) {
    ...
    (t.players || []).forEach(function (p) {
      ...
      if (tot.runs >= nextR - 150 && tot.runs > 0) rows.push({ nm: p.name, label: nextR.toLocaleString() + " club runs", cur: tot.runs, target: nextR, pct: Math.round(100 * tot.runs / nextR) });
      var nextW = (Math.floor(tot.wkts / 25) + 1) * 25;
      if (tot.wkts >= nextW - 8 && tot.wkts > 0) rows.push({ nm: p.name, label: nextW + " club wickets", cur: tot.wkts, target: nextW, pct: Math.round(100 * tot.wkts / nextW) });
      ...
      else if (tot.hs >= 30 && tot.hs < 50) rows.push({ nm: p.name, label: "maiden fifty (best " + tot.hs + ")", cur: tot.hs, target: 50, pct: Math.round(100 * tot.hs / 50) });
    });
    rows.sort(function (a, b) { return b.pct - a.pct; });
    return rows.slice(0, max || 3);
  }
