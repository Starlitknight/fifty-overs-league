...
(function () {
  ...
  function ensureNav() {
    try {
      ...
      if (after && after.nextSibling) tb.insertBefore(a, after.nextSibling); else tb.appendChild(a);
    } catch (e) {}
  }
  ...
  // Clubs & rosters: every club in the league with its players' BASIC info only
  // (name, role, age, country — no skills). Pull from the live league teams.
  function renderClubs() {
    openWrap(true); setNavy(false);
    wrap.querySelector("#folWho").textContent = LG ? LG.name : "";
    var teams = (typeof GD !== "undefined" && GD.teams) ? GD.teams : [];
    var roleName = function (r) { try { return (typeof window.prole === "function") ? window.prole(r) : (r || ""); } catch (e) { return r || ""; } };
    var body;
    if (!teams.length) {
      body = '<div class="folcard"><div class="folpad folsmall">No clubs yet. Start or open a season to see the league\'s clubs.</div></div>';
    } else {
      body = teams.map(function (t) {
        var rows = (t.players || []).map(function (p) {
          return "<tr><td>" + E(p.name) + "</td><td>" + E(roleName(p.role)) + "</td><td>" + (p.age || "-") + "</td><td>" + E(p.nat || "-") + "</td></tr>";
        }).join("") || '<tr><td colspan=4 class="folsmall">No players.</td></tr>';
        return '<div class="folcard"><h4>' + E(t.name) + " <span class='folsmall'>" + (t.players ? t.players.length : 0) + " players</span></h4>" +
          '<div class="folpad" style="overflow-x:auto"><table><tr><th>Player</th><th>Role</th><th>Age</th><th>Nat</th></tr>' + rows + "</table></div></div>";
      }).join("");
    }
    main.innerHTML = '<div class="folbody">' +
      '<div class="folcard"><div class="folpad folrow"><button class="mini" data-act="backToGame">◂ back to the game</button><span class="folsmall">Every club\'s roster (basic info)</span></div></div>' +
      body + "</div>";
  }
