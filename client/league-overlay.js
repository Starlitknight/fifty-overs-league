...
(function () {
  ...
  function ensureNav() {
    try {
      ...
      var a = mk("Clubs", "fo-clubs", renderClubs);
      var f = mk("Friendly", "fo-friendly", startFriendly);
      var lg = mk("League", "fo-league", openLeagueMenu);   // menu: admin / practice / log out
      if (after && after.nextSibling) { tb.insertBefore(a, after.nextSibling); tb.insertBefore(f, a.nextSibling); tb.insertBefore(lg, f.nextSibling); }
      else { tb.appendChild(a); tb.appendChild(f); tb.appendChild(lg); }
    } catch (e) {}
  }
  ...
  function practice() {
    sel("league_clubs", "league_id=eq." + LG.id + "&manager_id=eq." + SYNC.myMid + "&select=club").then(function (rows) {
      ...
      myClub.founded = true;                        // treat my club as the human club for balancing
      var world = fillBots([myClub]);
      GD.teams = world;
