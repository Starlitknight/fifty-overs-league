...
import {
  ...
  MATCHDAY_NET, PRIZE_PLAYOFF_CHAMP
...
export function seasonOf(opts) {
  const {
    ...
    playoffRounds = 0, playoffWin = false
  } = opts;
  ...
  for (let r = 1; r <= rounds; r++) {
    ...
    const med = Math.round(mediaSeason * r / rounds) - Math.round(mediaSeason * (r - 1) / rounds);
    const gIn = Math.round((Math.round(spG * r / rounds) - Math.round(spG * (r - 1) / rounds)) * sponsorMult);
