...
for (const n of NATS) {
  ...
  sides.forEach((side, slot) => {
    let players;
    try {
      // THE SEED CARRIES THE NATION AND THE TIER IS THE SERVER'S OWN RULE.
      // An earlier cut used 7000+slot and side.tier - which sidesOf does not
      // carry - so every nation's slot-N club was dealt the SAME untiered
      // fifteen, and a "3,840 cricketer" population was 240 men repeated
      // sixteen times. It moved the measured spin share by a point and a half.
      const seed = `${rid}|${slot}`;
      const div = side.div || (slot < 8 ? 1 : 2);
      const ixT = side.boss ? 5 : (div === 1 ? (slot <= 3 ? 4 : 3) : (slot <= 11 ? 2 : 1));
      const tier = ['newcomer', 'd2b', 'd2a', 'd1b', 'd1a', 'flagship'][Math.max(0, Math.min(5, ixT))];
